...
// 配置常量
export const SIGNATURE_CACHE_TTL_MS = 60 * 60 * 1000 // 1 小时（同 CLIProxyAPI）
export const MAX_ENTRIES_PER_SESSION = 100 // 每会话最大缓存条目
export const MIN_SIGNATURE_LENGTH = 50 // 最小有效签名长度
...
export const cacheSignature = function cacheSignature(sessionId, thinkingText, signature) {
  ...
  if (sessionCache.size >= MAX_ENTRIES_PER_SESSION) {
    ...
    logger.debug(`[SignatureCache] Evicted ${toRemove} old entries for session ${sessionId.slice(0, 8)}...`)
  }
  ...
  logger.debug(`[SignatureCache] Cached signature for session ${sessionId.slice(0, 8)}..., hash ${textHash}`)
}
...
export const getCachedSignature = function getCachedSignature(sessionId, thinkingText) {
  ...
  logger.debug(`[SignatureCache] Cache hit for session ${sessionId.slice(0, 8)}..., hash ${textHash}`)
