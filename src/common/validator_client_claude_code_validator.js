...
import { bestSimilarityByTemplates, SYSTEM_PROMPT_THRESHOLD } from '../modules/relay/relay_contents.js'
import * as metadataUserIdHelper from '../modules/relay/relay_metadata_user_id_helper.js'
import { env } from '../../config/env.js'

// 0 = 所有条目都必须匹配（默认严格模式），N > 0 = 至少 N 条匹配即可通过
const parsedMinMatches = parseInt(env.CLAUDE_CODE_MIN_SYSTEM_PROMPT_MATCHES)
const MIN_SYSTEM_PROMPT_MATCHES = Number.isInteger(parsedMinMatches) && parsedMinMatches >= 0 ? parsedMinMatches : 0
...
export class ClaudeCodeValidator {
  ...
  static hasClaudeCodeSystemPrompt(body, customThreshold, minMatchCount) {
    ...
    const minRequired =
      typeof minMatchCount === 'number' && Number.isInteger(minMatchCount) && minMatchCount > 0 ? minMatchCount : 0
    ...
    logger.debug(`Claude system prompt not detected: matchCount=${matchCount}, minRequired=${minRequired}`)
