...
// Cookie自动授权配置常量
export const COOKIE_OAUTH_CONFIG = {
...
export const parseCallbackUrl = function parseCallbackUrl(input) {
  ...
  if (trimmedInput.startsWith('http://') || trimmedInput.startsWith('https://')) {
    try {
      ...
      throw new Error('无效的 URL 格式，请检查回调 URL 是否正确', { cause: error })
    }
  }
...
export const exchangeSetupTokenCode = async function exchangeSetupTokenCode(
  ...
  try {
    ...
    if (error.response) {
      ...
      throw new Error('Setup Token exchange failed: No response from server (network error or timeout)', {
        cause: error,
      })
    } else {
      ...
      throw new Error(`Setup Token exchange failed: ${error.message}`, { cause: error })
    }
  }
}
  ...
  try {
    ...
    if (error.response) {
      ...
      if (status === 403 || status === 401) {
        throw new Error('Cookie授权失败：无效的sessionKey或已过期', { cause: error })
      }

      if (status === 302) {
        throw new Error('请求被Cloudflare拦截，请稍后重试', { cause: error })
      }

      throw new Error(`获取组织信息失败：HTTP ${status}`, { cause: error })
    } else if (error.request) {
      throw new Error('获取组织信息失败：网络错误或超时', { cause: error })
    }
...
export const authorizeWithCookie = async function authorizeWithCookie(
  ...
  // 构建授权URL
  const authorizeUrl = COOKIE_OAUTH_CONFIG.AUTHORIZE_URL_TEMPLATE.replace('{organization_uuid}', organizationUuid)
  ...
  try {
    ...
    if (error.response) {
      ...
      throw new Error(`授权请求失败：${errorMessage}`, { cause: error })
    } else if (error.request) {
      throw new Error('授权请求失败：网络错误或超时', { cause: error })
    }
