...
const ADDON_DIRECTORIES = [path.join(process.cwd(), '.local', 'ext'), path.join(process.cwd(), '.local', 'extensions')]
...
class RuntimeAddonBus {
  ...
  _log(level, message, error) {
    ...
    const loggerMethod = logger && typeof logger[targetLevel] === 'function' ? logger[targetLevel].bind(logger) : null
...
export const runtimeAddon = new RuntimeAddonBus()
