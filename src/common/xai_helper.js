...
export const OAUTH_ENDPOINT_ALLOWED_HOSTS = ['x.ai', '*.x.ai']
...
export const effectiveAuthorizeUrl = () => xaiConfig().oauthAuthorizeUrl || XAI_DEFAULTS.authorizeUrl
export const effectiveTokenUrl = () => xaiConfig().oauthTokenUrl || XAI_DEFAULTS.tokenUrl
export const effectiveClientId = () => xaiConfig().oauthClientId || XAI_DEFAULTS.clientId
export const effectiveScope = () => xaiConfig().oauthScope || XAI_DEFAULTS.scope

export const effectiveRedirectUri = (override = '') => {
...
export const effectiveBaseUrl = (override = '') => {
  const trimmed = String(override || '').trim()
  if (trimmed) {
...
export const effectiveCliBaseUrl = () => (xaiConfig().cliBaseUrl || XAI_DEFAULTS.cliBaseUrl).replace(/\/+$/, '')
...
export const isOfficialBaseUrlHost = (host) => hostMatches(host, BASE_URL_ALLOWED_HOSTS)

export const isOfficialBaseUrl = (raw) => {
...
export const normalizeKnownBaseUrlPath = (raw) => {
  let parsed
  try {
    parsed = new URL(raw)
  } catch {
...
export const validateOAuthEndpointUrl = (raw) => {
...
export const validateBaseUrl = (raw) => {
  if (allowUnsafeUrlOverrides()) {
...
export const validateTrustedBaseUrl = (raw) => {
  if (allowUnsafeUrlOverrides()) {
    return normalizeKnownBaseUrlPath(String(raw || '').trim())
  }
...
export const buildResponsesUrl = (baseUrl, validator = null) =>
  joinBasePath(validatedBaseUrl(baseUrl, validator), '/responses')

export const buildChatCompletionsUrl = (baseUrl, validator = null) =>
  joinBasePath(validatedBaseUrl(baseUrl, validator), '/chat/completions')

export const buildImagesGenerationsUrl = (baseUrl, validator = null) =>
  joinBasePath(validatedBaseUrl(baseUrl, validator), '/images/generations')

export const buildImagesEditsUrl = (baseUrl, validator = null) =>
  joinBasePath(validatedBaseUrl(baseUrl, validator), '/images/edits')

export const buildVideosGenerationsUrl = (baseUrl, validator = null) =>
  joinBasePath(validatedBaseUrl(baseUrl, validator), '/videos/generations')

export const buildVideosEditsUrl = (baseUrl, validator = null) =>
  joinBasePath(validatedBaseUrl(baseUrl, validator), '/videos/edits')

export const buildVideosExtensionsUrl = (baseUrl, validator = null) =>
  joinBasePath(validatedBaseUrl(baseUrl, validator), '/videos/extensions')

export const buildVideoUrl = (baseUrl, requestId, validator = null) => {
...
export const buildVideoContentUrl = (baseUrl, requestId, validator = null) =>
  `${buildVideoUrl(baseUrl, requestId, validator)}/content`

export const buildBillingUrl = (baseUrl, weekly = false, validator = null) => {
...
export const generateCodeChallenge = (verifier) => crypto.createHash('sha256').update(verifier).digest('base64url')

export const buildAuthorizationUrl = ({ state, codeChallenge, redirectUri, nonce } = {}) => {
...
export const parseAuthorizationInput = (raw) => {
  const trimmed = String(raw || '').trim()
  if (!trimmed) {
...
export const defaultModelIds = () => DEFAULT_MODELS.map((model) => model.id)

export const defaultModelMapping = () => {
  const mapping = {}
