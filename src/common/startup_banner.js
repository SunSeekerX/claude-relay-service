...
export const getLanIPv4Candidates = (interfaces = os.networkInterfaces()) => {
  const priv = []
  const other = []
  const isPrivate = (ip) =>
...
export const toUrlHost = (host) => (host && host.includes(':') ? `[${host}]` : host)
...
export const isLoopbackHost = (host) => {
  ...
  if (parsed.embeddedV4 && g[0] === 0 && g[1] === 0 && g[2] === 0 && g[3] === 0 && g[4] === 0 && g[5] === 0xffff) {
