...
import { RATE_LIMITED_MODEL_FAMILIES } from '../modules/relay/relay_model_helper.js'
...
export const createEncryptor = (salt) => {
...
export const { encrypt } = defaultEncryptor
export const { decrypt } = defaultEncryptor
export const getEncryptionKey = defaultEncryptor.getKey
export const clearDecryptCache = defaultEncryptor.clearCache
export const getDecryptCacheStats = defaultEncryptor.getStats
...
export const isTruthy = (value) => value !== null && value !== undefined && toBoolean(value)

// 检查是否可调度（默认 true，只有明确 false 才返回 false）
export const isSchedulable = (value) => value !== false && value !== 'false'

// 检查是否激活
export const isActive = (value) => value === true || value === 'true'

// 检查账户是否健康（激活且状态正常）
export const isAccountHealthy = (account) => {
  if (!account) {
    return false
  }
...
// 安全解析 JSON 为对象
export const safeParseJsonObject = (value, fallback = null) => {
...
// 规范化端点类型
export const normalizeEndpointType = (endpointType) => {
  if (!endpointType) {
    return 'anthropic'
  }
...
export const isAutoProtectionDisabled = (account) =>
  !!account && (account.disableAutoProtection === true || account.disableAutoProtection === 'true')

// 按优先级和最后使用时间排序账户
export const sortAccountsByPriority = (accounts) =>
  [...accounts].sort((a, b) => {
...
// 生成粘性会话 Key
export const composeStickySessionKey = (prefix, sessionHash, apiKeyId = null) => {
  if (!sessionHash) {
    return null
  }
...
// 过滤可用账户（激活 + 健康 + 可调度）
export const filterAvailableAccounts = (accounts) =>
...
export const READONLY_ACCOUNT_STATE_FIELDS = new Set([
  'status',
  'errorMessage',
  'rateLimitStatus',
  'rateLimitedAt',
...
export const stripReadonlyAccountFields = (updates) => {
...
// 掩码敏感信息（保留前后几位）
export const maskSensitive = (str, keepStart = 4, keepEnd = 4, maskChar = '*') => {
...
// 版本比较: a >= b
export const versionGte = (a, b) => a === b || versionGt(a, b)
