 ...
 */
export class CodexCliValidator {
  /**
   * 获取客户端ID
   */
  static getId() {
  ...
  static isCodexClientRequest(req) {
    ...
    const requireOriginator =
      ...
      lowerPath.startsWith('/backend-api/codex') ||
  ...
  static validate(req) {
    try {
      ...
      const strictValidationPaths = ['/openai', '/azure', '/backend-api/codex']
      const lowerFull = fullPath.toLowerCase()
      const needsStrictValidation =
        strictValidationPaths.some((prefix) => lowerFull.startsWith(prefix)) ||
        CodexCliValidator.isResponsesPath(fullPath) ||
        lowerFull === '/v1/models' ||
        lowerFull.startsWith('/v1/models')
      ...
      // 官方路径与 /openai、/azure：与 isCodexClientRequest 完全同源，禁止再分叉
      if (!CodexCliValidator.isCodexClientRequest(req)) {
        logger.debug(`Codex CLI validation failed - isCodexClientRequest=false path=${fullPath} UA=${userAgent}`)
        return false
      }
