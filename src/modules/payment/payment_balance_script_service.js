...
class BalanceScriptService {
  ...
  async execute(options = {}) {
    ...
    try {
      ...
      throw new Error(`脚本解析失败: ${error.message}`, { cause: error })
    }
    ...
    try {
      ...
      throw new Error(`extractor 执行失败: ${error.message}`, { cause: error })
    }
    ...
    const quota =
      Number.isFinite(total) || Number.isFinite(used)
        ? {
            ...
            percentage: Number.isFinite(total) && total > 0 && Number.isFinite(used) ? (used / total) * 100 : null,
          }
        : null
...
export const balanceScriptService = new BalanceScriptService()
