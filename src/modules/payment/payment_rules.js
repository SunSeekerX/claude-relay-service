...
// 实付金额 = 售价 ×(1 + 手续费率)
export const computePayAmount = (price, feeRate = 0) => round2(Number(price) * (1 + Number(feeRate)))

// 自定义额度换算售价 = 额度 × 倍率
export const computePriceFromQuota = (quota, ratio) => round2(Number(quota) * Number(ratio))

// 可回收额度 = min(订单额度, 当前余额)，余额为负按 0 计
export const computeRefundableQuota = (quotaAmount, balance) =>
  Math.max(0, Math.min(Number(quotaAmount) || 0, Math.max(Number(balance) || 0, 0)))

// 可退金额 = 售价 × 可回收额度 / 订单额度（按已消费比例对称退款，规避问题5）
export const computeRefundAmount = (price, quotaAmount, refundableQuota) =>
  Number(quotaAmount) > 0 ? round2((Number(price) * Number(refundableQuota)) / Number(quotaAmount)) : 0
