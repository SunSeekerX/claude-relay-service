...
import { balanceLedger } from './payment_balance_ledger.js'
import { registry } from './payment_registry.js'
import { providerRepository } from './payment_provider_repository.js'
import { paymentAudit } from './payment_audit.js'
import * as rules from './payment_rules.js'
...
class PaymentOrderService {
  ...
  async createOrder({ apiKeyId, planId = '', customQuota = 0, paymentType }) {
    ...
    try {
      ...
      try {
        ...
        const { count, pendingPayAmountSum } = await orderRepository.pendingStats(apiKeyId, reservedDate)
      ...
      logger.info(`[payment] order created id=${order.id} type=${paymentType} payAmount=${payAmount}`)
  ...
  async confirmPayment(order, { paidAmount = null, tradeNo = '' } = {}) {
    ...
    if (transitioned === 1) {
      const action =
        recoveredFrom === ORDER_STATUS.PENDING ? 'ORDER_PAID' : `ORDER_RECOVERED_FROM_${recoveredFrom.toUpperCase()}`
      ...
      if (!latest || (latest.status !== ORDER_STATUS.PAID && latest.status !== ORDER_STATUS.COMPLETED)) {
  ...
  async fulfillOrder(orderId) {
    ...
    // PAID -> COMPLETED
    const cas = await orderRepository.casStatus(order.id, ORDER_STATUS.PAID, ORDER_STATUS.COMPLETED, {
  ...
  async _ensureInstanceDailyReservation(order) {
    ...
    await providerRepository.restoreDailyReservation(
      ...
      order.id,
      order.providerReservedDate || (order.createdAt ? String(order.createdAt).slice(0, 10) : undefined),
      order.payAmount,
      Date.now(),
    )
  }
  ...
  async verifyOrder(orderId) {
    ...
    const recoverable = new Set([ORDER_STATUS.PENDING, ORDER_STATUS.EXPIRED, ORDER_STATUS.CANCELLED])
  ...
  async manualComplete(orderId, { operator = 'admin', reason = '', tradeNo = '' } = {}) {
    ...
    if (order.status === ORDER_STATUS.PAID) {
      await paymentAudit.record(orderId, 'ORDER_MANUAL_FULFILL', { reason: trimmedReason }, operator)
      await this.fulfillOrder(orderId)
    ...
    while (processed < MAX_PER_TICK) {
      ...
      for (const id of ids) {
        ...
        if (cas === 1) {
          ...
          await paymentAudit.record(order.id, queryFailed ? 'ORDER_EXPIRED_QUERY_FAILED' : 'ORDER_EXPIRED', {})
        }
      }
      if (ids.length < BATCH) {
        break
  ...
  async _approveRefundLocked(orderId, operator) {
    ...
    const locked = await orderRepository.casStatus(order.id, ORDER_STATUS.COMPLETED, ORDER_STATUS.REFUNDING, {})
    if (locked !== 1) {
      throw new Error('订单当前状态不允许退款')
    }

    // ① 先原子回收额度：Lua 内按【执行时】派生余额裁剪（防快照过时超退）；0=已被新消费吃光
    const reversed = await balanceLedger.reverse(order.apiKeyId, refundableQuota, `${order.id}:refund`, {
      orderId: order.id,
      operator,
    })
  ...
  async resolveRefundInDoubt(orderId, outcome, { operator = 'admin' } = {}) {
    ...
    try {
      ...
      if (outcome === 'not_refunded' && order.channelRefundedAt) {
        throw new Error('该订单已标记渠道已退款，仅可「判已退」补终态；若标记有误属数据异常，请人工核实')
      }
      ...
      const rolledBack = await this._rollbackRefund(order, quota, operator, 'in-doubt resolved: channel not refunded')
      if (!rolledBack) {
  ...
  async _executeChannelRefund(order, reversed, actualRefundAmount, operator) {
    ...
    if (!refundResult.success) {
      await this._rollbackRefund(order, reversed, operator, refundResult.message || 'refund rejected')
  ...
  async _finalizeRefund(order, reversed, actualRefundAmount, operator) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        transitioned = await orderRepository.casStatus(order.id, ORDER_STATUS.REFUNDING, ORDER_STATUS.REFUNDED, {
  ...
  async cancelOrder(orderId, apiKeyId) {
    ...
    const ok = await orderRepository.casStatus(orderId, ORDER_STATUS.PENDING, ORDER_STATUS.CANCELLED, {})
    if (ok !== 1) {
  ...
  async dashboard() {
    const settledStatuses = new Set([ORDER_STATUS.COMPLETED, ORDER_STATUS.REFUNDING, ORDER_STATUS.REFUNDED])
