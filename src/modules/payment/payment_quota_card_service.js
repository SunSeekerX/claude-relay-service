...
class QuotaCardService {
   ...
   * 保存额度卡上限配置
   */
  async saveLimitsConfig(limitsConfig) {
    try {
      const parsedDays = parseInt(limitsConfig.maxExpiryDays)
      const parsedCost = parseFloat(limitsConfig.maxTotalCostLimit)
      const newConfig = {
        enabled: limitsConfig.enabled !== false,
      ...
      if (card.type === 'time' || card.type === 'combo') {
        ...
        if (limits.enabled && limits.maxExpiryDays > 0) {
          ...
          if (new Date(result.newExpiresAt) > maxExpiry) {
            await redis.client.hset(RedisKeys.apiKey.byId(apiKeyId), 'expiresAt', maxExpiry.toISOString())
    ...
    try {
      ...
      if (keyword) {
        redemptions = redemptions.filter((item) =>
          [item.cardCode, item.username, item.userId, item.apiKeyName, item.apiKeyId].some((field) =>
            (field || '').toLowerCase().includes(keyword),
          ),
        )
      }
...
export const quotaCardService = new QuotaCardService()
