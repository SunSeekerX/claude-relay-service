...
export class StripeProvider extends BasePaymentProvider {
  ...
  async createPayment(order, config) {
    ...
    const { data } = await axios.post(`${STRIPE_API}/checkout/sessions`, querystring.stringify(form), {
      timeout: 15000,
      headers: {
  ...
  async queryOrder(outTradeNo, config, order = null) {
    ...
    if (sessionId.startsWith('cs_')) {
      try {
        const { data } = await axios.get(`${STRIPE_API}/checkout/sessions/${encodeURIComponent(sessionId)}`, auth)
        session = data
      } catch (e) {
