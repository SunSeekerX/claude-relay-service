...
import { BasePaymentProvider } from './payment_base_payment_provider.js'
...
const bjTimestamp = () => new Date(Date.now() + 8 * 3600 * 1000).toISOString().slice(0, 19).replace('T', ' ')
...
const buildSignStr = (params) =>
  Object.keys(params)
    .filter(
      (k) => k !== 'sign' && k !== 'sign_type' && params[k] !== '' && params[k] !== undefined && params[k] !== null,
    )
    .sort()
    .map((k) => `${k}=${params[k]}`)
    .join('&')
...
export class AlipayProvider extends BasePaymentProvider {
  ...
  async _call(method, bizContent, config) {
    ...
    const { data } = await axios.post(config.gateway || DEFAULT_GATEWAY, new URLSearchParams(params).toString(), {
      timeout: 15000,
  ...
  async verifyWebhook(rawBody, headers, config, query) {
    ...
    const success = verified && (data.trade_status === 'TRADE_SUCCESS' || data.trade_status === 'TRADE_FINISHED')
    return {
  ...
  async refund(order, amount, config) {
    ...
    if (!resp || resp.code === '20000') {
      throw new Error(`alipay refund result unknown: ${resp ? resp.sub_msg || resp.msg : 'empty response'}`)
    }
    return {
