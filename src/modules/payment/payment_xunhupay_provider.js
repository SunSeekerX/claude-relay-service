...
export class XunhupayProvider extends BasePaymentProvider {
  get providerKey() {
    return 'xunhupay'
  }
  ...
  async queryOrder(outTradeNo, config) {
    try {
      ...
      params.hash = xunhuSign(params, config.appSecret)
      const gateway = config.gateway || 'https://api.xunhupay.com'
      const { data } = await axios.post(`${gateway}/payment/query.html`, querystring.stringify(params), {
        timeout: 10000,
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      })
  ...
  async refund(order, amount, config) {
    ...
    const { data } = await axios.post(`${gateway}/payment/refund.html`, querystring.stringify(params), {
      timeout: 15000,
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    })
