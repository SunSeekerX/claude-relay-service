...
export class WxpayProvider extends BasePaymentProvider {
  get providerKey() {
    return 'wxpay'
  }
  ...
  _authHeader(method, urlPath, body, config) {
    ...
    const signature = crypto.createSign('RSA-SHA256').update(message, 'utf8').sign(toPem(config.privateKey), 'base64')
