...
const ALL_STATUSES = ['pending', 'paid', 'completed', 'expired', 'cancelled', 'failed', 'refunding', 'refunded']

const NUMERIC_FIELDS = ['quotaAmount', 'price', 'payAmount', 'feeRate', 'paidAmount', 'refundedAmount']
...
class OrderRepository {
    ...
    if (status) {
      ...
      for (const drift of drifts) {
        ...
        if (drift.order && drift.order.status && ALL_STATUSES.includes(drift.order.status)) {
          const score = drift.order.createdAt ? new Date(drift.order.createdAt).getTime() : Date.now()
  ...
  async listByApiKey(apiKeyId, { offset = 0, limit = 20 } = {}) {
    ...
    const ids = await redis.client.zrevrange(APIKEY_IDX(apiKeyId), safeOffset, safeOffset + safeLimit - 1)
    const orders = await this._loadOrdersByIds(ids)
  ...
  async pendingStats(apiKeyId, sumDate) {
    ...
    for (let i = 0; i < ids.length; i += PIPELINE_CHUNK) {
      ...
      for (const row of results || []) {
        ...
        if (status === 'pending') {
          count += 1
          if (!sumDate || (createdAt && redis.getDateStringInTimezone(new Date(createdAt)) === sumDate)) {
