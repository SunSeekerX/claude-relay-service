...
export const ProxyStatus = { ENABLED: 1, DISABLED: 2, FAULT: 3 }

// 熔断器状态机
export const CircuitState = { CLOSED: 'closed', OPEN: 'open', HALF_OPEN: 'half_open' }

// 错误类型（用于异常检测与降权告警）
export const ProxyErrorType = {
  TIMEOUT: 'timeout',
...
// 服务层启动时注入配置
export const configureCore = (partial) => {
...
export class CircuitBreaker {
  ...
  recordFailure() {
    ...
    if (this.state === CircuitState.CLOSED && this.failureCount >= CORE_CONFIG.circuitFailureThreshold) {
      this.state = CircuitState.OPEN
      this.openedAt = now
    }
  }
  ...
  checkHalfOpen() {
    if (this.state === CircuitState.OPEN && Date.now() - this.openedAt >= CORE_CONFIG.circuitCooldownMs) {
...
export class ProxyRuntimeState {
...
export const computeHealthFactor = (successRate1m) => {
...
export const computeLatencyFactor = (p99Ms) => {
  if (p99Ms <= 0) {
    return 1.0
  }
  if (p99Ms <= 200) {
...
export const computeSuccessRateFactor = (rate5m) => {
  if (rate5m >= 0.99) {
    return 1.0
  }
  if (rate5m >= 0.95) {
...
export const computeDynamicWeight = (baseWeight, state, aggregated) => {
  // 熔断断开 -> 权重 0（先尝试进入半开）
  ...
  const successRateFactor = stats5m.requests > 0 ? computeSuccessRateFactor(stats5m.successRate) : 1.0

  const weight = Math.round(baseWeight * healthFactor * latencyFactor * successRateFactor * slowStartFactor)
  return Math.max(0, Math.min(100, weight))
}

// 跨分片聚合：把多个 worker 的同一 proxyId:contextKey 状态合并为全局视图
export const aggregateShardStats = (states, now = Date.now()) => {
...
export const buildWeightedProxy = (config, state) => {
  if (!state) {
    return {
      id: config.id,
      url: config.url,
...
export const buildRouteEntry = (proxies) => {
...
export const isProxyTransportOk = (success, statusCode) => {
...
export const classifyProxyErrorFromStatus = (statusCode) => {
  if (statusCode === 403 || statusCode === 429) {
...
export const classifyProxyError = (statusCode, error) => {
  if (statusCode) {
...
export const validateProxyUrl = (url) => typeof url === 'string' && PROXY_URL_REGEX.test(url.trim())
...
export const validateContextKey = (contextKey) => typeof contextKey === 'string' && CONTEXT_KEY_REGEX.test(contextKey)

// 平台名归一化为合法 contextKey（连字符转下划线，截断 30 字符）
export const normalizeContextKey = (platform) => {
