...
import { ProxyHelper } from './proxy_helper.js'
import { proxyPoolService } from './proxy_pool_service.js'
import { classifyProxyError, classifyProxyErrorFromStatus } from './proxy_pool_core.js'
...
class ProxyHealthService {
  ...
  async _tick() {
    ...
    const acquired = await redis.setAccountLock(RedisKeys.proxy.healthCheckLock, lockToken, lockTtlMs)
    if (!acquired) {
      return
    }
  ...
  async checkProxy(proxyConfig) {
    ...
    proxyPoolService.recordResult(proxyConfig.id, 'health_check', healthy, latencyMs, statusCode, errorType, true)
    await redis
