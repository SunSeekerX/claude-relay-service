...
import * as proxyPoolCore from './proxy_pool_core.js'
...
class ProxyPoolService {
  constructor() {
    ...
    this.proxyStates = new Map() // "proxyId:contextKey" -> proxyPoolCore.ProxyRuntimeState
    this.localVersion = 0
  ...
  // 把归一化后的设置写进 config（健康字段 + core 字段），不触发 proxyPoolCore.configureCore（由调用方决定时机）
  ...
  // 启动时把 Redis 持久化设置合并进 config（proxyPoolCore.configureCore 由 start() 统一调用）
  ...
  async reloadFromRedis() {
    ...
    for (const proxyConfig of this.proxyConfigs.values()) {
      proxyConfig.groupIds = reverseIndex.has(proxyConfig.id) ? Array.from(reverseIndex.get(proxyConfig.id)) : []
    }
  }
  ...
  _isEligible(proxyConfig) {
    return proxyConfig && proxyConfig.status === proxyPoolCore.ProxyStatus.ENABLED && proxyConfig.isHealthy
  }
  ...
  _setFallback(groupId, proxyIds) {
    ...
    for (const proxyId of proxyIds) {
      ...
      if (this._isEligible(proxyConfig)) {
        proxies.push(proxyPoolCore.buildWeightedProxy(proxyConfig))
      }
    }
    if (proxies.length > 0) {
      this.fallbackTable.set(groupId, proxyPoolCore.buildRouteEntry(proxies))
    } else {
  ...
  rebuildContextRouteEntry(groupId, contextKey) {
    ...
    for (const proxyId of proxyIds) {
      ...
      if (state) {
        state.weight = proxyPoolCore.computeDynamicWeight(proxyConfig.baseWeight, state)
      }
      const weightedProxy = proxyPoolCore.buildWeightedProxy(proxyConfig, state)
    ...
    if (proxies.length > 0) {
      this.routeTable.set(routeKey, proxyPoolCore.buildRouteEntry(proxies))
    } else {
  ...
  rebuildProxyRouteEntries(proxyId, contextKey) {
    const proxyConfig = this.proxyConfigs.get(proxyId)
    const affectedGroups = proxyConfig && proxyConfig.groupIds ? [...proxyConfig.groupIds, ALL_GROUP] : [ALL_GROUP]
    if (contextKey) {
    ...
    for (const groupId of affectedGroups) {
      ...
      this._setFallback(
        groupId,
        groupId === ALL_GROUP ? new Set(this._allProxyIds()) : this.groupProxyIds.get(groupId) || new Set(),
      )
    }
  }
  ...
  getAvailableProxies(groupId, contextKey) {
    ...
    const resolvedContextKey = proxyPoolCore.validateContextKey(contextKey)
      ? contextKey
      : proxyPoolCore.normalizeContextKey(contextKey)
  ...
  sampleProxy(groupId, contextKey) {
    ...
    const resolvedContextKey = proxyPoolCore.validateContextKey(contextKey)
      ...
      : proxyPoolCore.normalizeContextKey(contextKey)
    const entry =
      this.routeTable.get(`${resolvedGroupId}:${resolvedContextKey}`) || this.fallbackTable.get(resolvedGroupId)
    if (!entry) {
      return null
    }
  ...
  getProxy(proxyId, contextKey) {
    ...
    if (!state) {
      ...
      return proxyPoolCore.buildWeightedProxy(proxyConfig)
    }
    if (state.circuitBreaker.state === proxyPoolCore.CircuitState.OPEN) {
      state.circuitBreaker.checkHalfOpen()
    }
    state.weight = proxyPoolCore.computeDynamicWeight(proxyConfig.baseWeight, state)
    return proxyPoolCore.buildWeightedProxy(proxyConfig, state)
  }

  // === 对外接口（写，fire-and-forget） ===
  recordResult(proxyId, contextKey, success, latencyMs, statusCode, errorType, successIsTransport = false) {
    const proxyConfig = this.proxyConfigs.get(proxyId)
    if (!proxyConfig) {
      return
    }
    ...
    const isNew = !state
    if (!state) {
      state = new proxyPoolCore.ProxyRuntimeState(proxyId, resolvedContextKey)
    ...
    const transportOk = successIsTransport ? success : proxyPoolCore.isProxyTransportOk(success, statusCode)
    const resolvedErrorType = transportOk
      ? undefined
      : errorType || proxyPoolCore.classifyProxyErrorFromStatus(statusCode)
    state.record(transportOk, latencyMs, resolvedErrorType)

    // 半开探针完成 -> 成功则慢启动恢复
    if (
      oldCircuitState === proxyPoolCore.CircuitState.HALF_OPEN &&
      state.circuitBreaker.state === proxyPoolCore.CircuitState.CLOSED
    ) {
    ...
    const weightChanged =
      Math.abs(state.weight - oldWeight) >= proxyPoolCore.CORE_CONFIG.weightChangeThreshold ||
  ...
  async createProxy(input) {
    if (!proxyPoolCore.validateProxyUrl(input.url)) {
    ...
    const proxyConfig = {
      id: proxyId,
      name: input.name || proxyPoolCore.maskProxyUrl(input.url),
      url: input.url.trim(),
      status: proxyPoolCore.ProxyStatus.ENABLED,
      isHealthy: true,
  ...
  async updateProxy(proxyId, updates) {
    ...
    if (updates.url !== undefined && updates.url.trim() !== proxyPoolCore.maskProxyUrl(proxyConfig.url)) {
      if (!proxyPoolCore.validateProxyUrl(updates.url)) {
        throw new Error('Invalid proxy url')
      }
  ...
  _deserializeActiveShards(proxyId, contextKey, shardMap) {
    ...
    for (const json of Object.values(shardMap || {})) {
      const state = proxyPoolCore.ProxyRuntimeState.deserialize(proxyId, contextKey, json)
      if (now - state.lastActive <= proxyPoolCore.CORE_CONFIG.shardActiveMs) {
        states.push(state)
      }
    }
    return states
  ...
  _mergeShards(proxyId, _contextKey, states) {
    ...
    const proxyConfig = this.proxyConfigs.get(proxyId)
    if (proxyConfig) {
      const aggregated = proxyPoolCore.aggregateShardStats(states)
      base.weight = proxyPoolCore.computeDynamicWeight(proxyConfig.baseWeight, base, aggregated)
    }
    return base
  }
  ...
  getEnabledProxyConfigs() {
    return Array.from(this.proxyConfigs.values()).filter(
      (proxyConfig) => proxyConfig.status === proxyPoolCore.ProxyStatus.ENABLED,
    )
  }
