...
class UnifiedClaudeScheduler {
          ...
          if (!allowDedicatedFallback) {
            ...
            if (boundAccount && !isAutoProtectionDisabled(boundAccount)) {
              ...
              if (
                ...
                (await claudeAccountService.isAccountModelRateLimited(boundAccount.id, requestedModelFamily))
              ) {
                const info = await claudeAccountService.getAccountModelRateLimitInfo(
                  boundAccount.id,
                  requestedModelFamily,
                )
                const error = new Error(`Dedicated Claude account reached its ${requestedModelFamily} model limit`)
                error.code = 'CLAUDE_DEDICATED_RATE_LIMITED'
                error.accountId = boundAccount.id
            ...
            } else if (await this.isAccountTemporarilyUnavailable(boundAccount.id, 'claude-official')) {
  ...
  async _getAllAvailableAccounts(apiKeyData, requestedModel = null, includeCcr = false) {
    ...
    for (const account of claudeAccounts) {
      ...
      if (
        ...
        (autoOff || (account.status !== 'error' && account.status !== 'blocked' && account.status !== 'temp_error'))
      ) {
    ...
    for (const account of consoleAccounts) {
      ...
      if (currentAccount.status === 'quota_exceeded') {
        ...
        const isStillExceeded = await claudeConsoleAccountService.isAccountQuotaExceeded(currentAccount.id)
        if (!isStillExceeded) {
          // 重新获取账户最新状态
      ...
      if (
        ...
        try {
          ...
          logger.warn(`Failed to check quota for Claude Console account ${currentAccount.name}: ${e.message}`)
          // 继续处理该账号
        }
    ...
    if (bedrockAccountsResult.success) {
      ...
      for (const account of bedrockAccounts) {
        ...
        const autoOff = isAutoProtectionDisabled(account)
        if (account.isActive === true && account.accountType === 'shared' && isSchedulable(account.schedulable)) {
    ...
    if (includeCcr) {
      ...
      for (const account of ccrAccounts) {
        ...
        if (
          ...
          // 检查是否被限流（开关 ON 时跳过限流过滤）
          const isRateLimited = autoOff ? false : await ccrAccountService.isAccountRateLimited(account.id)
          // 检查是否超额（预算，始终校验——方案甲：开关不覆盖预算）
  ...
  async _isAccountAvailable(accountId, accountType, requestedModel = null) {
    try {
      if (accountType === 'claude-official') {
        ...
        // 检查模型兼容性
        if (!this._isModelSupportedByAccount(account, 'claude-official', requestedModel, 'in session check')) {
          return false
        }
        ...
        if (!autoProtectionOff) {
          // 检查账户状态
          if (account.status !== 'active' && account.status !== 'unauthorized' && account.status !== 'overloaded') {
            return false
          }
        }
        // 检查模型支持（始终校验）
        if (!this._isModelSupportedByAccount(account, 'claude-console', requestedModel, 'in session check')) {
          return false
        }
        // 检查订阅是否过期（本地约束，始终校验）
    ...
    try {
      ...
      for (const account of ccrAccounts) {
        ...
        if (
          ...
          const isRateLimited = autoOff ? false : await ccrAccountService.isAccountRateLimited(account.id)
          const isQuotaExceeded = await ccrAccountService.isAccountQuotaExceeded(account.id)
          const isOverloaded = autoOff ? false : await ccrAccountService.isAccountOverloaded(account.id)
  ...
  async _isAccountAvailableForSessionBinding(accountId, accountType, _requestedModel = null) {
    try {
      ...
      const autoProtectionOff = account.disableAutoProtection === true || account.disableAutoProtection === 'true'
      if (!autoProtectionOff) {
        if (status === 'error' || status === 'temp_error') {
          logger.warn(`Session binding: Claude OAuth account ${accountId} has error status: ${status}`)
          return false
        }
...
export const unifiedClaudeScheduler = new UnifiedClaudeScheduler()
