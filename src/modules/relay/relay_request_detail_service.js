...
const createCostRecomputePatch = function createCostRecomputePatch(record = {}) {
  ...
  try {
    ...
    const pricingSource =
      costResult?.debug?.pricingSource || (costResult?.usingDynamicPricing ? 'dynamic' : 'unknown-fallback')
...
const createRequestDetailDateBoundarySignature = function createRequestDetailDateBoundarySignature(
  ...
  if (type === 'start') {
    const floorDate = boundaryValue instanceof Date ? boundaryValue : new Date(boundaryValue || Date.now())
  ...
  if (type === 'end') {
    const ceilingDate = boundaryValue instanceof Date ? boundaryValue : new Date(boundaryValue || Date.now())
...
const requestDetailFilterSignaturesMatch = function requestDetailFilterSignaturesMatch(
  ...
  return (
    ...
    requestDetailDateBoundarySignaturesMatch(normalizedSnapshot.endBoundary, normalizedCurrent.endBoundary, 'end')
  )
}
...
const updateAvailableFilterAccumulator = function updateAvailableFilterAccumulator(accumulator, record) {
  ...
  if (record.accountId) {
    accumulator.accountMap.set(record.accountId, {
      ...
      accountTypeName: record.accountTypeName || accountTypeNames[record.accountType] || accountTypeNames.unknown,
    })
  }
...
class RequestDetailService {
  ...
  async _enrichRecords(records = [], apiKeyCache = new Map(), accountCache = new Map()) {
    ...
    for (const record of records) {
      ...
      const cacheMetrics = requestDetailHelper.getRequestDetailCacheMetrics(displayRecord)
      const reasoningInfo = requestDetailHelper.resolveRequestDetailReasoning(displayRecord)
      ...
      enriched.push({
        ...
        accountTypeName:
          accountInfo?.accountTypeName || accountTypeNames[displayRecord.accountType] || accountTypeNames.unknown,
  ...
  _hydrateRawRecord(rawItem, pointer = {}) {
    const parsed = restoreRecordTimestamp(safeJsonParse(rawItem), Number(pointer?.timestampMs) || Date.now())
  ...
  async _buildListQueryData(filters, effectiveStart, effectiveEnd, sortOrder) {
    ...
    if (hasKeyword) {
      ...
      for (let startIndex = 0; startIndex < requestPointers.length; startIndex += REQUEST_DETAIL_QUERY_BATCH_SIZE) {
        const pointerBatch = requestPointers.slice(startIndex, startIndex + REQUEST_DETAIL_QUERY_BATCH_SIZE)
        ...
        enrichedBatch.forEach((record, index) => {
          ...
          if (!this._matchesStructuredFilters(record, filters) || !this._matchesKeyword(record, filters.keyword)) {
            return
          }
  ...
  async _loadQuerySnapshot(snapshotId, filterSignature, client = redis.getClient()) {
    ...
    if (!parsedSnapshot || !requestDetailFilterSignaturesMatch(parsedSnapshot.filterSignature, filterSignature)) {
      return null
    }
  ...
  async listRequestDetails(filters = {}) {
    ...
    const responseFilters = this._buildResponseFilters(filters, effectiveStart, effectiveEnd, sortOrder)
    const filterSignature = createRequestDetailFilterSignature(
      filters,
      {
        ...
        endBoundary: createRequestDetailDateBoundarySignature('end', filters.endDate, effectiveEnd, now),
      },
    ...
    const queryData = await this._buildListQueryData(filters, effectiveStart, effectiveEnd, sortOrder)
    ...
    const snapshotId = await this._storeQuerySnapshot(filterSignature, queryData, responseFilters, sortOrder)
...
export const requestDetailService = new RequestDetailService()
