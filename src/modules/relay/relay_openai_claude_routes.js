...
export const handleChatCompletion = async function handleChatCompletion(req, res, apiKeyData) {
  ...
  try {
    ...
    if (claudeRequest.stream) {
      ...
      const usageCallback = (usage) => {
        ...
        if (usage && usage.input_tokens !== undefined && usage.output_tokens !== undefined) {
          ...
          const requestBetaHeader =
            req.headers['anthropic-beta'] || req.headers['Anthropic-Beta'] || req.headers['ANTHROPIC-BETA']
      ...
      const streamTransformer = (chunk) => openaiToClaude.convertStreamChunk(chunk, req.body.model, sessionId)
