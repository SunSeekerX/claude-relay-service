...
export const handleAzureOpenAIRequest = async function handleAzureOpenAIRequest({
  ...
  try {
    ...
    const apiVersion = account.apiVersion || (endpoint === 'responses' ? '2025-04-01-preview' : '2024-02-01')
    ...
    if (proxyAgent) {
      ...
      logger.debug(`Using proxy for Azure OpenAI request: ${ProxyHelper.getProxyDescription(account.proxy)}`)
    }
    ...
    logger.debug('Azure OpenAI request headers', {
      ...
      customHeaders: Object.keys(requestHeaders).filter((key) => !['Content-Type', 'user-agent'].includes(key)),
    })
    ...
    } else if (error.code === 'CERT_AUTHORITY_INVALID' || error.code === 'UNABLE_TO_VERIFY_LEAF_SIGNATURE') {
...
export const handleStreamResponse = function handleStreamResponse(upstreamResponse, clientResponse, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // 透传某些头部
    const passThroughHeaders = ['x-request-id', 'x-ratelimit-remaining-requests', 'x-ratelimit-remaining-tokens']
