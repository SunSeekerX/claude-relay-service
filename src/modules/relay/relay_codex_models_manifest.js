...
import codexClientModelsTemplate from '../pricing/pricing_codex_client_models.json' with { type: 'json' }
...
export const getDefaultCodexModelIds = () => [...templateBySlug.keys()]

// 合成 Codex ModelsResponse
// modelIds == null → 模板全量；[] → 空目录（不回退）
export const buildCodexModelsManifest = (modelIds = null) => {
...
export const buildOpenAIModelsList = (modelIds) => {
...
export const codexManifestEtag = (manifest) =>
  `W/"${crypto.createHash('sha256').update(JSON.stringify(manifest)).digest('hex').slice(0, 32)}"`

export const codexManifestETagMatches = (ifNoneMatch, etag) => {
  if (!ifNoneMatch || !etag) {
    return false
  }
  const normalize = (value) => {
    const trimmed = String(value).trim()
    return trimmed.length >= 2 && trimmed.slice(0, 2).toUpperCase() === 'W/' ? trimmed.slice(2).trim() : trimmed
  }
