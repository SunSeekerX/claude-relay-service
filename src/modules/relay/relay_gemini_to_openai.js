...
export class GeminiToOpenAIConverter {
  ...
  convertResponse(geminiData, model) {
    ...
    const choices = candidates.map((candidate, i) => {
      ...
      for (const part of parts) {
        if (part.thoughtSignature && !part.text && !part.functionCall && !part.inlineData && !part.inline_data) {
          continue
        }
  ...
  _mapFinishReason(geminiReason) {
    ...
    if (fr === 'malformed_function_call' || fr === 'too_many_tool_calls' || fr === 'unexpected_tool_call') {
      return 'stop'
    }
