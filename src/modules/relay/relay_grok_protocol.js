...
export const stripSsePingFrames = (chunkText) => {
...
export const messagesToResponsesInput = (messages) => {
  const input = []
...
export const chatToResponsesBody = (body) => {
  const mapped = {
    model: body.model,
...
export const maybeInjectFreeCacheTools = (responsesBody, account) => {
  if (!account) {
    return responsesBody
  }
  const enabled = account.grokClientToolCacheEnabled === true || account.grokClientToolCacheEnabled === 'true'
  if (!enabled) {
    return responsesBody
  }
  ...
  const isFree = plan === 'free' || plan === 'basic' || (plan.includes('free') && !plan.includes('super'))
  if (!isFree && plan !== '') {
    return responsesBody
  }
  return {
...
export const buildGrokCompactRequestBody = (body) => {
  const payload = { ...(body || {}) }
  let { input } = payload
...
export const convertGrokResponseToOpenAICompact = (responseBody) => {
...
// failover 判定
export const shouldFailoverGrokStatus = (statusCode) => {
  if (!statusCode) {
    return false
  }
...
export const isInvalidEncryptedContentError = (statusCode, body) => {
  if (statusCode !== 400) {
    return false
  }
  let code = ''
  ...
  return (
    normalized.includes('encrypted_content') && (normalized.includes('decrypt') || normalized.includes('unmodified'))
  )
}

// 剥离 reasoning.encrypted_content 以便同号重试
export const stripEncryptedReasoningContent = (body) => {
  if (!body || typeof body !== 'object') {
...
export const videoSessionHash = (requestId, apiKeyId) =>
  crypto
    .createHash('sha256')
