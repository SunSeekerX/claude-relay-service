  ...
  switch (rule.valueType) {
    ...
    case 'json':
      try {
        ...
        throw new Error(`Rule path "${rule.path}" expects valid JSON`, { cause: error })
      }
    default:
...
export const applyRules = function applyRules(body, rules) {
  ...
  let nextBody = typeof structuredClone === 'function' ? structuredClone(body) : JSON.parse(JSON.stringify(body))
