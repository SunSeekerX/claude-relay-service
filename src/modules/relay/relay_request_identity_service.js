...
const headersChanged = function headersChanged(original, updated) {
  ...
  for (const key of STAINLESS_HEADER_KEYS) {
    if (getHeaderValueCaseInsensitive(original, key) !== getHeaderValueCaseInsensitive(updated, key)) {
      return true
    }
  }
...
const rewriteHeaders = function rewriteHeaders(headers, accountId) {
  ...
  if (fieldCount < MIN_FINGERPRINT_FIELDS) {
    logger.warn(`requestIdentityService: 账号 ${accountId} 提供的 Stainless 指纹字段不足，已保持原样`)
...
export const transform = function transform(payload = {}) {
  ...
  const abortResponse = headerResult && headerResult.abortResponse ? headerResult.abortResponse : null
