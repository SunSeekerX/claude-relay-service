...
import {
  handleStandardGenerateContent as geminiHandleGenerateContent,
  handleStandardStreamGenerateContent as geminiHandleStreamGenerateContent,
} from './relay_gemini_handlers_routes.js'
  ...
  if (backend === 'claude') {
    ...
    if (req.body.stream) {
      ...
      res.write = function (chunk, encoding, callback) {
        ...
        const converted = geminiConverter.convertStreamChunk(chunk, requestedModel, geminiStreamState)
        if (converted) {
...
const buildGeminiRequestFromOpenAI = function buildGeminiRequestFromOpenAI(body) {
  ...
  if (body.tools && body.tools.length > 0) {
    ...
    for (const tool of body.tools) {
      ...
      } else if (tool.type === 'google_search' || tool.type === 'code_execution' || tool.type === 'url_context') {
