...
const getUserMessageQueueService = () => {
  if (!_userMessageQueueService) {
    _userMessageQueueService = require('../user/user_message_queue_service.js').userMessageQueueService
  }
  return _userMessageQueueService
}
...
class BedrockRelayService {
  constructor() {
    this.defaultRegion = env.AWS_REGION || config.bedrock?.defaultRegion || 'us-east-1'
    this.smallFastModelRegion = env.ANTHROPIC_SMALL_FAST_MODEL_AWS_REGION || this.defaultRegion

    // 默认模型配置
    this.defaultModel = env.ANTHROPIC_MODEL || 'us.anthropic.claude-sonnet-4-20250514-v1:0'
    this.defaultSmallModel = env.ANTHROPIC_SMALL_FAST_MODEL || 'us.anthropic.claude-3-5-haiku-20241022-v1:0'

    // Token配置 — 仅作为客户端未指定 max_tokens 时的回退默认值，不用于截断
    this.maxOutputTokens = parseInt(env.BEDROCK_MAX_OUTPUT_TOKENS) || 128000
  ...
  _getBedrockClient(region = null, bedrockAccount = null) {
    ...
    const { agent: proxyAgent, proxyId, contextKey } = proxyResolver.resolveAgent(bedrockAccount, 'bedrock')
  ...
  // 处理流式请求
  async handleStreamRequest(requestBody, bedrockAccount = null, res, _req = null) {
    const accountId = bedrockAccount?.id
    let queueLockAcquired = false
    let queueRequestId = null
  ...
  _handleBedrockError(error, accountId = null, bedrockAccount = null, requestBody = null) {
    const autoProtectionDisabled =
      bedrockAccount?.disableAutoProtection === true || bedrockAccount?.disableAutoProtection === 'true'
    if (accountId && !autoProtectionDisabled) {
      ...
      if (error.name === 'ThrottlingException') {
        upstreamErrorHelper.markTempUnavailable(accountId, 'bedrock', 429, null, errorContext).catch(() => {})
      } else if (error.name === 'AccessDeniedException') {
        upstreamErrorHelper.markTempUnavailable(accountId, 'bedrock', 403, null, errorContext).catch(() => {})
      } else if (error.name === 'ServiceUnavailableException' || error.name === 'InternalServerException') {
        upstreamErrorHelper.markTempUnavailable(accountId, 'bedrock', 500, null, errorContext).catch(() => {})
      } else if (error.name === 'ModelNotReadyException') {
        upstreamErrorHelper.markTempUnavailable(accountId, 'bedrock', 503, null, errorContext).catch(() => {})
      }
    }
...
export const bedrockRelayService = new BedrockRelayService()
