...
import { webhookService } from '../webhook/webhook_service.js'
...
class RateLimitCleanupService {
  ...
  async performCleanup() {
    ...
    try {
      ...
      const totalChecked =
        results.openai.checked + results.claude.checked + results.claudeConsole.checked + results.quotaExceeded.checked
      const totalCleared =
        results.openai.cleared + results.claude.cleared + results.claudeConsole.cleared + results.quotaExceeded.cleared
  ...
  async cleanupClaudeAccounts(result) {
    try {
      ...
      for (const account of accounts) {
        ...
        const needsAutoStopRecovery = autoStopped && (account.rateLimitEndAt || account.schedulable === 'false')
      ...
      for (const account of accounts) {
        ...
        if (isRateLimited || hasStatusRateLimited || needsAutoStopRecovery) {
          ...
          try {
            ...
            const isStillLimited = await claudeConsoleAccountService.isAccountRateLimited(account.id)
      ...
      for (const account of accounts) {
        ...
        if (account.status === 'quota_exceeded' || account.quotaStoppedAt) {
          ...
          try {
            ...
            const isStillExceeded = await claudeConsoleAccountService.isAccountQuotaExceeded(account.id)
