...
export const TEMP_UNAVAILABLE_PREFIX = RedisKeys.upstream.tempUnavailablePrefix
export const ERROR_HISTORY_PREFIX = RedisKeys.upstream.errorHistoryPrefix
...
const getConfig = () => {
  if (!_configCache) {
    try {
      _configCache = require('../../config/config.js').config
    } catch {
      _configCache = {}
    }
  }
...
const getRelayConfigService = () => {
  if (!_relayConfigService) {
    _relayConfigService = require('./relay_claude_relay_config_service.js').claudeRelayConfigService
  }
...
// 根据 HTTP 状态码分类错误类型
export const classifyError = (statusCode) => {
  if (statusCode === 529) {
    return 'overload'
  }
  if (statusCode === 503) {
...
export const buildErrorContext = (raw) => {
  ...
  try {
    ...
    const errorBody = sanitizeAndStringifyBody(raw.responseBody ?? raw.errorBody, limits.responseBodyMax)
    if (errorBody) {
...
// 记录错误历史到 Redis List
export const recordErrorHistory = async (accountId, accountType, statusCode, errorType, context = null) => {
...
// 标记账户为临时不可用
export const markTempUnavailable = async (
  accountId,
  accountType,
  statusCode,
  customTtl = null,
  ...
  try {
    ...
    if (Number.isFinite(policyDecision.ttlOverrideSeconds) && policyDecision.ttlOverrideSeconds > 0) {
...
// 检查账户是否临时不可用
export const isTempUnavailable = async (accountId, accountType) => {
  try {
    const redis = getRedis()
    const client = redis.getClientSafe()
    const key = RedisKeys.upstream.tempUnavailable(accountType, accountId)
...
// 清除临时不可用状态
export const clearTempUnavailable = async (accountId, accountType) => {
  try {
    const redis = getRedis()
    const client = redis.getClientSafe()
    const key = RedisKeys.upstream.tempUnavailable(accountType, accountId)
...
export const getAllTempUnavailable = async () => {
  try {
    const redis = getRedis()
    const client = redis.getClientSafe()
...
export const hasAutoStopEvidence = (accountData = {}) => {
  ...
  return AUTO_STOP_MARKER_FIELDS.some((field) => accountData[field] === 'true' || accountData[field] === true)
}

// 判断账户是否存在模型家族限流残留（opus/sonnet/haiku/fable）。
// 仅表示"某模型被限流"，不代表整个账号被停用——因此与 hasAutoStopEvidence 分开。
export const hasModelFamilyRateLimit = (accountData = {}) =>
...
export const clearAutoProtectionCooldowns = async (accountId, accountType) => {
...
export const sanitizeErrorForClient = (errorData) => {
