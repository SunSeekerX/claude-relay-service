...
router.post('/claude-console-accounts', authenticateAdmin, async (req, res) => {
  try {
    ...
    // 校验上游错误自动防护开关
    const normalizedDisableAutoProtection = disableAutoProtection === true || disableAutoProtection === 'true'
    ...
    const newAccount = await claudeConsoleAccountService.createAccount({
      ...
      maxConcurrentTasks:
        maxConcurrentTasks !== undefined && maxConcurrentTasks !== null ? Number(maxConcurrentTasks) : 0,
  ...
  try {
    ...
    if (mappedUpdates.disableAutoProtection !== undefined) {
      mappedUpdates.disableAutoProtection =
        mappedUpdates.disableAutoProtection === true || mappedUpdates.disableAutoProtection === 'true'
    }
