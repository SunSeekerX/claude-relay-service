...
class GeminiApiAccountService {
  ...
  async createAccount(options = {}) {
    ...
    const accountData = {
      ...
      // 自动防护开关
      disableAutoProtection: disableAutoProtection === true || disableAutoProtection === 'true' ? 'true' : 'false',
    }
  ...
  async updateAccount(accountId, updates) {
    ...
    // 规范化 baseUrl
    if (updates.baseUrl) {
      updates.baseUrl = updates.baseUrl.endsWith('/') ? updates.baseUrl.slice(0, -1) : updates.baseUrl
    }
...
export const geminiApiAccountService = new GeminiApiAccountService()
