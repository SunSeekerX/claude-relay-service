...
// 平台 -> 账户服务（getAccount / getAllAccounts 分发用）
const ACCOUNT_SERVICE_BY_PLATFORM = {
...
export class AccountBalanceService {
  ...
  async getAllAccountsBalance(platform, options = {}) {
    ...
    const results = await this._mapWithConcurrency(accounts, this.DEFAULT_CONCURRENCY, async (acc) => {
      try {
  ...
  async getBalanceSummary() {
    ...
    for (const platform of platforms) {
      ...
      const balances = await this._mapWithConcurrency(accounts, this.DEFAULT_CONCURRENCY, async (acc) => {
  ...
  async getAccount(accountId, platform) {
    ...
    const service = ACCOUNT_SERVICE_BY_PLATFORM[platform]
  ...
  async _getAccountBalanceForAccount(account, platform, options = {}) {
    ...
    const effectiveQueryMode =
      queryMode === 'auto' && !(platform === 'gemini' && account?.oauthProvider === 'antigravity') ? 'local' : queryMode
  ...
  _buildResponse(balanceData, accountId, platform, source, ttlSeconds = null, extraData = {}) {
    ...
    if (source === 'cache') {
      const ttl = typeof ttlSeconds === 'number' && ttlSeconds > 0 ? ttlSeconds : this.CACHE_TTL_SECONDS
