...
class OpenAIResponsesAccountService {
  ...
  async createAccount(options = {}) {
    const {
      ...
      // 模型映射：{ 客户端模型: 上游模型 }，空={} 不改名
      supportedModels = null,
      // 模型白名单：客户端模型 ID 列表，空=[] 不限制
      allowedModels = null,
      // 最大并发任务数，0=不限制
    ...
    if (!validEndpoints.includes(providerEndpoint)) {
      throw new Error(`Invalid providerEndpoint: ${providerEndpoint}. Must be one of: ${validEndpoints.join(', ')}`)
    }
    ...
    const accountData = {
      ...
      // 模型映射（JSON 对象）；模型白名单（JSON 数组）
      supportedModels: JSON.stringify(this._normalizeSupportedModels(supportedModels)),
      allowedModels: JSON.stringify(this._normalizeAllowedModels(allowedModels)),
      maxConcurrentTasks: String(Number(maxConcurrentTasks) || 0),
    }
    ...
    return {
      ...accountData,
      apiKey: '***', // 返回时隐藏敏感信息
      supportedModels: this._parseSupportedModels(accountData.supportedModels),
      allowedModels: this._parseAllowedModels(accountData.allowedModels),
      maxConcurrentTasks: parseInt(accountData.maxConcurrentTasks, 10) || 0,
    }
  }
  ...
  async getAccount(accountId) {
    ...
    accountData.allowedModels = this._parseAllowedModels(accountData.allowedModels)
  ...
  async updateAccount(accountId, updates) {
    ...
    if (updates.supportedModels !== undefined) {
      updates.supportedModels = JSON.stringify(this._normalizeSupportedModels(updates.supportedModels))
    }
    if (updates.allowedModels !== undefined) {
      updates.allowedModels = JSON.stringify(this._normalizeAllowedModels(updates.allowedModels))
    }

    if (updates.maxConcurrentTasks !== undefined && updates.maxConcurrentTasks !== null) {
      const concurrent = Number(updates.maxConcurrentTasks)
      if (!Number.isInteger(concurrent) || concurrent < 0) {
        throw new Error('maxConcurrentTasks must be a non-negative integer')
      }
      updates.maxConcurrentTasks = concurrent.toString()
    }
  ...
  // supportedModels：仅模型映射 {from:to}；空={} 不改名
  _normalizeSupportedModels(supportedModels) {
    if (!supportedModels) {
      return {}
    }
    if (Array.isArray(supportedModels)) {
      // 兼容旧：数组当自映射，但新语义下数组应走 allowedModels
      const mapping = {}
  ...
  _parseSupportedModels(raw) {
    if (!raw) {
      return {}
    }
    if (typeof raw === 'object' && !Array.isArray(raw)) {
      return this._normalizeSupportedModels(raw)
    }
    if (typeof raw === 'string') {
      try {
        const parsed = JSON.parse(raw)
        return this._normalizeSupportedModels(parsed)
      } catch {
        return {}
      }
    }
  ...
  // allowedModels：白名单数组；空=[] 不限制
  _normalizeAllowedModels(allowedModels) {
    if (!allowedModels) {
      return []
    }
    if (Array.isArray(allowedModels)) {
      return allowedModels.filter((m) => typeof m === 'string' && m.trim()).map((m) => m.trim())
    }
    if (typeof allowedModels === 'object') {
      // 兼容误传对象
      return Object.keys(allowedModels).filter((k) => k.trim())
    }
    return []
  }

  _parseAllowedModels(raw) {
    if (!raw) {
      return []
    }
    if (Array.isArray(raw)) {
      return this._normalizeAllowedModels(raw)
    }
    if (typeof raw === 'string') {
      try {
        return this._normalizeAllowedModels(JSON.parse(raw))
      } catch {
        return []
      }
    }
  ...
  // 编辑回填兼容：旧数据 supportedModels 全是自映射时拆成白名单
  hydrateModelFields(account) {
    if (!account) {
      return account
    }
    let mapping = this._parseSupportedModels(account.supportedModels)
    let allowed = this._parseAllowedModels(account.allowedModels)
    const keys = Object.keys(mapping)
    if (allowed.length === 0 && keys.length > 0) {
      const allSelf = keys.every((k) => mapping[k] === k)
      if (allSelf) {
        allowed = [...keys]
        mapping = {}
      }
...
export const openaiResponsesAccountService = new OpenAIResponsesAccountService()
