...
import { createEncryptor, isModelInMapping, getMappedModelName } from '../../common/common_helper.js'
...
class CcrAccountService {
  ...
  async createAccount(options = {}) {
    ...
    logger.debug(`[DEBUG] Saving CCR account data to Redis with key: ${RedisKeys.accounts.ccr(accountId)}`)
  ...
  async updateAccount(accountId, updates) {
    try {
      ...
      // 处理各个字段的更新
      logger.debug(`[DEBUG] CCR update request received with fields: ${Object.keys(updates).join(', ')}`)
  ...
  async deleteAccount(accountId) {
    try {
      ...
      logger.success(`Deleted CCR account: ${accountId}`)
      return { success: true }
    } catch (error) {
  ...
  async markAccountRateLimited(accountId) {
    try {
      ...
      if (account.rateLimitDuration === 0) {
        logger.info(`ℹ CCR account ${account.name} (${accountId}) has rate limiting disabled, skipping rate limit`)
        return { success: true, skipped: true }
      }
      ...
      logger.warn(`Marked CCR account as rate limited: ${account.name} (${accountId})`)
  ...
  async removeAccountRateLimit(accountId) {
    try {
      ...
      if (quotaStoppedAt) {
        ...
        logger.info(`ℹ CCR account ${accountId} rate limit removed but remains stopped due to quota exceeded`)
      } else {
  ...
  async isAccountRateLimited(accountId) {
    try {
      const client = redis.getClientSafe()
      const accountKey = RedisKeys.accounts.ccr(accountId)
      const [rateLimitedAt, rateLimitDuration] = await client.hmget(accountKey, 'rateLimitedAt', 'rateLimitDuration')
  ...
  // 检查模型是否被支持 — 委托公共 helper，含 prefix* 通配
...
export const ccrAccountService = new CcrAccountService()
