...
import { logRefreshStart, logRefreshSuccess, logRefreshError } from '../../common/token_refresh_logger.js'
...
const OAUTH_SESSION_TTL_SECONDS = Math.floor((xaiHelper.XAI_DEFAULTS.sessionTtlMs || 30 * 60 * 1000) / 1000)
...
class GrokAccountService {
  ...
  async _requestToken({
    ...
    if (response.status < 200 || response.status >= 300) {
      ...
      const err = new Error(`Grok OAuth token request failed: status ${response.status}, body: ${body.slice(0, 500)}`)
  ...
  _normalizeTokenInfo(tokenResp, clientId = '') {
    const expiresIn =
      Number(tokenResp.expires_in) > 0 ? Number(tokenResp.expires_in) : Math.floor(DEFAULT_ACCESS_TOKEN_TTL_MS / 1000)
  ...
  async createAccount(options = {}) {
    ...
    try {
      ...
      throw new Error(`Invalid base URL: ${error.message}`, { cause: error })
    }
  ...
  async updateAccount(accountId, updates = {}) {
    ...
    const enablingAutoProtection = next.disableAutoProtection === 'true' && existing.disableAutoProtection !== 'true'
  ...
  _hydrateListAccount(accountData) {
    ...
    data.disableAutoProtection = data.disableAutoProtection === true || data.disableAutoProtection === 'true'
    data.platform = 'grok'
  ...
  getUpstreamBaseUrl(account, { media = false } = {}) {
    ...
    return account.authType === 'oauth' ? xaiHelper.effectiveCliBaseUrl() : xaiHelper.effectiveBaseUrl()
  }
  ...
  async queryQuota(accountId) {
    ...
    // 尽量从响应头提取订阅档
    const tierHeader =
      weeklyRes?.headers?.['xai-subscription-tier'] || monthlyRes?.headers?.['xai-subscription-tier'] || ''
    const entitlementHeader =
      weeklyRes?.headers?.['xai-entitlement-status'] || monthlyRes?.headers?.['xai-entitlement-status'] || ''
  ...
  runtimeSanity() {
    ...
    return {
      ...
      oauthAuthorizeUrl: check(xaiHelper.effectiveAuthorizeUrl(), (v) => xaiHelper.validateOAuthEndpointUrl(v)),
      oauthTokenUrl: check(xaiHelper.effectiveTokenUrl(), (v) => xaiHelper.validateOAuthEndpointUrl(v)),
      oauthRedirectUri: {
  ...
  getMediaGenerationEligibility(account) {
    ...
    const plan = String(account.planType || account.subscriptionTier || account.subscriptionTier || '')
      .trim()
      .toLowerCase()
    // Free / basic 明确拒绝
    if (
  ...
  async ensureMediaEligible(accountId) {
    ...
    if (result.reason === 'billing_unobserved' && account.authType === 'oauth') {
      try {
        ...
        const isFree =
          !tier || tier === 'free' || tier === 'basic' || (tier.includes('free') && !tier.includes('super'))
        const forbidden =
        ...
        // 有成功月度/周度观测且非 free
        const hasObservation = quota.monthly && !quota.monthly.error ? true : quota.weekly && !quota.weekly.error
        if (!hasObservation) {
  ...
  async createAccountsFromSSO({
    ...
    const worker = async () => {
      while (cursor < tokens.length) {
        ...
        try {
          const accountName =
            name && tokens.length === 1 ? name : name ? `${name}-${index + 1}` : `Grok SSO ${index + 1}`
...
export const grokAccountService = new GrokAccountService()
