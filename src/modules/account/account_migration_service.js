...
import * as mappers from './account_migration_mappers.js'
import * as parsers from './account_migration_parsers.js'
import crypto from 'node:crypto'

export { CRS_PLATFORM } from './account_migration_mappers.js'
/**
...
const readClaudeSnapshot = async function readClaudeSnapshot(id) {
  ...
  return {
    platform: mappers.CRS_PLATFORM.CLAUDE,
    id,
    name: a.name || '',
...
const readOpenAiSnapshot = async function readOpenAiSnapshot(id) {
  ...
  return {
    platform: mappers.CRS_PLATFORM.OPENAI,
    id,
    name: a.name || '',
    description: a.description || '',
    data: {
...
const readGeminiSnapshot = async function readGeminiSnapshot(id) {
  ...
  return {
    platform: mappers.CRS_PLATFORM.GEMINI,
    id,
    name: a.name || '',
    description: a.description || '',
    data: {
...
const readGeminiApiSnapshot = async function readGeminiApiSnapshot(id) {
  ...
  return apiKeySnapshot(mappers.CRS_PLATFORM.GEMINI_API, id, a, {
    apiKey: a.apiKey,
    baseUrl: a.baseUrl,
    supportedModels: a.supportedModels || [],
  })
...
const readClaudeConsoleSnapshot = async function readClaudeConsoleSnapshot(id) {
  ...
  return apiKeySnapshot(mappers.CRS_PLATFORM.CLAUDE_CONSOLE, id, a, {
    apiUrl: a.apiUrl,
    apiKey: a.apiKey,
    supportedModels: a.supportedModels || [],
...
const readCcrSnapshot = async function readCcrSnapshot(id) {
  ...
  return apiKeySnapshot(mappers.CRS_PLATFORM.CCR, id, a, {
    apiUrl: a.apiUrl,
    apiKey: a.apiKey,
    supportedModels: a.supportedModels || [],
    userAgent: a.userAgent,
...
const readOpenAiResponsesSnapshot = async function readOpenAiResponsesSnapshot(id) {
  ...
  return apiKeySnapshot(mappers.CRS_PLATFORM.OPENAI_RESPONSES, id, a, {
    baseApi: a.baseApi,
    apiKey: a.apiKey,
    userAgent: a.userAgent,
    rateLimitDuration: a.rateLimitDuration,
...
const readAzureSnapshot = async function readAzureSnapshot(id) {
  ...
  return apiKeySnapshot(mappers.CRS_PLATFORM.AZURE_OPENAI, id, a, {
...
const readDroidSnapshot = async function readDroidSnapshot(id) {
  ...
  return {
    platform: mappers.CRS_PLATFORM.DROID,
    id,
    name: a.name || '',
    description: a.description || '',
    data: {
...
const readGrokSnapshot = async function readGrokSnapshot(id) {
  ...
  return {
    platform: mappers.CRS_PLATFORM.GROK,
    id,
    name: a.name || '',
    description: a.description || '',
    data: {
...
const readBedrockSnapshot = async function readBedrockSnapshot(id) {
  ...
  const a = res.data
  return {
    platform: mappers.CRS_PLATFORM.BEDROCK,
    id,
    name: a.name || '',
    description: a.description || '',
    data: {
...
const PLATFORM_READERS = {
  [mappers.CRS_PLATFORM.CLAUDE]: readClaudeSnapshot,
  [mappers.CRS_PLATFORM.CLAUDE_CONSOLE]: readClaudeConsoleSnapshot,
  [mappers.CRS_PLATFORM.CCR]: readCcrSnapshot,
  [mappers.CRS_PLATFORM.OPENAI]: readOpenAiSnapshot,
  [mappers.CRS_PLATFORM.OPENAI_RESPONSES]: readOpenAiResponsesSnapshot,
  [mappers.CRS_PLATFORM.GEMINI]: readGeminiSnapshot,
  [mappers.CRS_PLATFORM.GEMINI_API]: readGeminiApiSnapshot,
  [mappers.CRS_PLATFORM.AZURE_OPENAI]: readAzureSnapshot,
  [mappers.CRS_PLATFORM.BEDROCK]: readBedrockSnapshot,
  [mappers.CRS_PLATFORM.DROID]: readDroidSnapshot,
  [mappers.CRS_PLATFORM.GROK]: readGrokSnapshot,
}

// 各平台 ID 枚举（index + scan 回退 + 提取正则）
const PLATFORM_INDEX = {
  [mappers.CRS_PLATFORM.CLAUDE]: {
    index: RedisKeys.accounts.claudeIndex,
    pattern: RedisKeys.accounts.claudePattern,
    re: /^claude:account:(.+)$/,
  },
  [mappers.CRS_PLATFORM.CLAUDE_CONSOLE]: {
    index: RedisKeys.accounts.claudeConsoleIndex,
    pattern: RedisKeys.accounts.claudeConsolePattern,
    re: /^claude_console_account:(.+)$/,
  },
  [mappers.CRS_PLATFORM.CCR]: {
    index: RedisKeys.accounts.ccrIndex,
    pattern: RedisKeys.accounts.ccrPattern,
    re: /^ccr_account:(.+)$/,
  },
  [mappers.CRS_PLATFORM.OPENAI]: {
    index: RedisKeys.accounts.openaiIndex,
    pattern: RedisKeys.accounts.openaiPattern,
    re: /^openai:account:(.+)$/,
  },
  [mappers.CRS_PLATFORM.OPENAI_RESPONSES]: {
...
const snapshotFingerprint = function snapshotFingerprint(snap) {
  ...
  switch (snap.platform) {
    ...
    case mappers.CRS_PLATFORM.OPENAI: {
      const o = d.openaiOauth || {}
      const info = d.accountInfo || {}
      return fp('openai', info.email || '', info.accountId || '', o.refreshToken || d.refreshToken || '')
    }
    case mappers.CRS_PLATFORM.GEMINI: {
      const o = typeof d.geminiOauth === 'string' ? sj(d.geminiOauth, {}) : d.geminiOauth || {}
      return fp('gemini', d.oauthProvider || 'gemini-cli', d.projectId || '', o.refresh_token || d.refreshToken || '')
    }
    case mappers.CRS_PLATFORM.GEMINI_API:
      return fp('gemini-api', d.apiKey || '', d.baseUrl || '')
    case mappers.CRS_PLATFORM.CLAUDE_CONSOLE:
      return fp('claude-console', d.apiUrl || '', d.apiKey || '')
    case mappers.CRS_PLATFORM.CCR:
      return fp('ccr', d.apiUrl || '', d.apiKey || '')
    case mappers.CRS_PLATFORM.OPENAI_RESPONSES:
      return fp('openai-responses', d.baseApi || '', d.apiKey || '')
    case mappers.CRS_PLATFORM.AZURE_OPENAI:
      return fp('azure', d.azureEndpoint || '', d.deploymentName || '', d.apiKey || '')
    case mappers.CRS_PLATFORM.BEDROCK: {
      const c = d.awsCredentials || {}
      return fp('bedrock', d.credentialType || '', d.region || '', c.accessKeyId || d.bearerToken || '')
    }
    case mappers.CRS_PLATFORM.DROID: {
      // API key 模式账户 refreshToken 为空，用 apiKeys 集合做指纹（排序保证稳定）；
      // endpointType 决定路由行为，同 keys 不同端点是不同账户（兜底 anthropic 与读取侧一致）
      const apiKeys = Array.isArray(d.apiKeys) ? d.apiKeys.filter(Boolean).slice().sort().join(',') : ''
      return fp('droid', d.endpointType || 'anthropic', d.ownerEmail || '', d.userId || '', d.refreshToken || apiKeys)
    }
    case mappers.CRS_PLATFORM.GROK:
      return fp('grok', d.authType || 'oauth', d.email || '', d.refreshToken || d.apiKey || '', d.baseUrl || '')
    default:
      return ''
  }
}
...
const validateSnapshotWarnings = function validateSnapshotWarnings(snapshot) {
  ...
  if (snapshot.platform === mappers.CRS_PLATFORM.GEMINI_API && !d.apiKey) {
