...
// CRS 原生导出头
export const CRS_TYPE = 'crs-accounts'

// sub2api 头（含 legacy）。已核实 account_data.go: dataType / legacyDataType。
export const SUB2API_TYPES = new Set(['sub2api-data', 'sub2api-bundle'])

// CLIProxyAPI auth 文件已知 type。已核实各 TokenStorage.type。
export const CLIPROXY_TYPES = new Set(['claude', 'codex', 'gemini', 'antigravity', 'vertex', 'xai', 'kimi'])
...
export const extractJsonEntriesFromZip = function extractJsonEntriesFromZip(buffer) {
  ...
  for (const entry of zip.getEntries()) {
    ...
    if (!lower.endsWith('.json') || base.startsWith('.') || entry.entryName.startsWith('__MACOSX')) {
      continue
    }
    try {
...
export const detectFormatFromJson = function detectFormatFromJson(json) {
  ...
  if (Array.isArray(json.accounts)) {
    ...
    // CRS 账户条目带 data{}
    if (json.accounts.some((a) => a && typeof a === 'object' && a.data && typeof a.data === 'object')) {
      return FORMAT.CRS
    }
