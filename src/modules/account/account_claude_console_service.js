...
class ClaudeConsoleAccountService {
  ...
  _getBlockedHandlingMinutes() {
    const raw = env.CLAUDE_CONSOLE_BLOCKED_HANDLING_MINUTES
  ...
  async createAccount(options = {}) {
    ...
    logger.debug(`[DEBUG] Saving account data to Redis with key: ${RedisKeys.accounts.claudeConsole(accountId)}`)
  ...
  async removeAccountRateLimit(accountId) {
    try {
      ...
      // 获取账户当前状态和额度信息
      const [currentStatus, quotaStoppedAt] = await client.hmget(accountKey, 'status', 'quotaStoppedAt')
  ...
  async removeAccountOverload(accountId) {
    try {
      ...
      await client.hdel(RedisKeys.accounts.claudeConsole(accountId), 'overloadedAt', 'overloadStatus')
...
export const claudeConsoleAccountService = new ClaudeConsoleAccountService()
