...
import * as antigravityClient from '../relay/relay_antigravity_client.js'
...
const getOauthProviderConfig = function getOauthProviderConfig(oauthProvider, { requireCredentials = true } = {}) {
  ...
  if (hasClientId !== hasClientSecret) {
    throw new Error(`${envPrefix}_CLIENT_ID / ${envPrefix}_CLIENT_SECRET 必须成对配置，当前只配置了其中一个`)
  }

  if (requireCredentials && (!hasClientId || !hasClientSecret)) {
    throw new Error(`${envPrefix}_CLIENT_ID / ${envPrefix}_CLIENT_SECRET 未配置，无法进行 Gemini OAuth`)
  }
  return config
}
...
export const fetchAvailableModelsAntigravity = async function fetchAvailableModelsAntigravity(
  ...
  try {
    ...
    if (refreshToken) {
      try {
        const client = await getOauthClient(accessToken, refreshToken, proxyConfig, OAUTH_PROVIDER_ANTIGRAVITY)
...
export const refreshAccessToken = async function refreshAccessToken(
  ...
  try {
    ...
    throw new Error(`Failed to refresh access token: ${error.message}`, { cause: error })
  }
}
...
export const createAccount = async function createAccount(accountData) {
  ...
  if (accountData.geminiOauth || accountData.accessToken) {
    ...
    if (accountData.geminiOauth) {
      geminiOauth =
        typeof accountData.geminiOauth === 'string' ? accountData.geminiOauth : JSON.stringify(accountData.geminiOauth)

      const oauthData =
        typeof accountData.geminiOauth === 'string' ? JSON.parse(accountData.geminiOauth) : accountData.geminiOauth
...
export const updateAccount = async function updateAccount(accountId, updates) {
  ...
  if (updates.geminiOauth) {
    updates.geminiOauth = encrypt(
      typeof updates.geminiOauth === 'string' ? updates.geminiOauth : JSON.stringify(updates.geminiOauth),
    )
  }
  ...
  if (updates.geminiOauth && !oldRefreshToken) {
    const oauthData =
      typeof updates.geminiOauth === 'string' ? JSON.parse(decrypt(updates.geminiOauth)) : updates.geminiOauth
...
export const selectAvailableAccount = async function selectAvailableAccount(apiKeyId, sessionHash = null) {
  ...
  if (apiKeyData.geminiAccountId) {
    ...
    if (account && account.isActive === 'true') {
      ...
      // 记录token使用情况
      tokenRefreshLogger.logTokenUsage(account.id, account.name, 'gemini', account.expiresAt, isExpired)
  ...
  // 记录token使用情况
  tokenRefreshLogger.logTokenUsage(
    selectedAccount.id,
...
export const refreshAccountToken = async function refreshAccountToken(accountId) {
  ...
  try {
    ...
    // 传入账户的代理配置
    const newTokens = await refreshAccessToken(account.refreshToken, account.proxy, account.oauthProvider)
    ...
    // 记录刷新成功
    tokenRefreshLogger.logRefreshSuccess(accountId, account.name, 'gemini', {
      accessToken: newTokens.access_token,
    ...
    // 记录刷新失败
    tokenRefreshLogger.logRefreshError(accountId, account ? account.name : 'Unknown', 'gemini', error)
