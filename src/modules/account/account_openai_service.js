...
const buildCodexUsageSnapshot = function buildCodexUsageSnapshot(accountData) {
  ...
  const hasPrimaryData =
    primaryUsedPercent !== null || primaryResetAfterSeconds !== null || primaryWindowMinutes !== null
  const hasSecondaryData =
    secondaryUsedPercent !== null || secondaryResetAfterSeconds !== null || secondaryWindowMinutes !== null
...
export const refreshAccountToken = async function refreshAccountToken(accountId) {
  ...
  try {
    ...
    if (!refreshToken) {
      tokenRefreshLogger.logRefreshSkipped(accountId, accountName, 'openai', 'No refresh token available')
    ...
    tokenRefreshLogger.logRefreshSuccess(accountId, accountName, 'openai', newTokens) // 传入完整的 newTokens 对象
    return newTokens
  } catch (error) {
    tokenRefreshLogger.logRefreshError(accountId, account?.name || accountName, 'openai', error.message)
...
export const createAccount = async function createAccount(accountData) {
  ...
  if (accountData.openaiOauth) {
    oauthData =
      typeof accountData.openaiOauth === 'string' ? JSON.parse(accountData.openaiOauth) : accountData.openaiOauth
  }
  ...
  const isEmailEncrypted = accountInfo.email && accountInfo.email.length >= 33 && accountInfo.email.charAt(32) === ':'
  ...
  const account = {
    ...
    accessToken: oauthData.accessToken && oauthData.accessToken.trim() ? encrypt(oauthData.accessToken) : '',
    refreshToken: oauthData.refreshToken && oauthData.refreshToken.trim() ? encrypt(oauthData.refreshToken) : '',
  ...
  if (accountData.proxy) {
    account.proxy = typeof accountData.proxy === 'string' ? accountData.proxy : JSON.stringify(accountData.proxy)
  }
...
export const updateAccount = async function updateAccount(accountId, updates) {
  ...
  if (updates.openaiOauth) {
    const oauthData =
      typeof updates.openaiOauth === 'string' ? updates.openaiOauth : JSON.stringify(updates.openaiOauth)
...
export const getAccountOverview = async function getAccountOverview(accountId) {
  ...
  const scopes = accountData.scopes && accountData.scopes.trim() ? accountData.scopes.split(' ') : []
...
export const selectAvailableAccount = async function selectAvailableAccount(apiKeyId, sessionHash = null) {
  ...
  if (apiKeyData.openaiAccountId) {
    ...
    if (account && account.isActive === 'true') {
      ...
      // 记录token使用情况
      tokenRefreshLogger.logTokenUsage(account.id, account.name, 'openai', account.expiresAt, isExpired)
