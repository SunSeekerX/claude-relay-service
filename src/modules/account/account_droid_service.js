...
class DroidAccountService {
  ...
  _parseApiKeyEntries(rawEntries) {
    ...
    if (typeof rawEntries === 'string') {
      try {
        ...
        logger.warn('Failed to parse Droid API Key entries:', error.message)
        return []
      }
    }
  ...
  async getDecryptedApiKeyEntries(accountId) {
    ...
    return entries.map((entry) => this._decryptApiKeyEntry(entry)).filter((entry) => entry && entry.key)
  }
  ...
  async touchApiKeyUsage(accountId, keyId) {
    ...
    try {
      ...
      updatedEntry.usageCount = String(Number.isFinite(usageCount) && usageCount >= 0 ? usageCount + 1 : 1)
      ...
      logger.warn(`Failed to update API key usage for Droid account ${accountId}:`, error)
    }
  }
  ...
  async removeApiKeyEntry(accountId, keyId) {
    ...
    try {
      ...
      logger.warn(`已删除 Droid API Key ${keyId}（Account: ${accountId}），剩余 ${filtered.length}`)
  ...
  async markApiKeyAsError(accountId, keyId, errorMessage = '') {
    ...
    try {
      ...
      if (accountData.disableAutoProtection === true || accountData.disableAutoProtection === 'true') {
        logger.info(`Droid 账号 ${accountId} 已关闭自动防护，跳过将 API Key ${keyId} 标记为异常`)
      ...
      logger.warn(`已标记 Droid API Key ${keyId} 为异常状态（Account: ${accountId}）：${errorMessage}`)
  ...
  async _refreshTokensWithWorkOS(refreshToken, proxyConfig = null, organizationId = null) {
    ...
    if (proxyConfig) {
      ...
      if (proxyAgent) {
        ...
        logger.info(`使用代理验证 Droid Refresh Token: ${ProxyHelper.getProxyDescription(proxyConfig)}`)
      }
    }
    ...
    const { access_token, refresh_token, user, organization_id, expires_in, token_type, authentication_method } =
      response.data

    let expiresAt = response.data.expires_at || ''
    if (!expiresAt) {
      const expiresInSeconds =
        typeof expires_in === 'number' && Number.isFinite(expires_in) ? expires_in : this.tokenValidHours * 3600
  ...
  async _fetchFactoryOrgIds(accessToken, proxyConfig = null) {
    ...
    try {
      ...
      logger.warn('未从 Factory CLI 接口获取到 workosOrgIds')
      return []
    } catch (error) {
      logger.warn('获取 Factory 组织信息失败:', error.message)
      return []
    }
  }
  ...
  async createAccount(options = {}) {
    ...
    const normalizedAuthMethod =
      typeof normalizedAuthenticationMethod === 'string' ? normalizedAuthenticationMethod.toLowerCase().trim() : ''
    ...
    if (isApiKeyProvision) {
      logger.info(`[Droid api_key] 初始密钥 - AccountName: ${name}, KeyCount: ${apiKeyEntries.length}`)
    } else {
      logger.info(
    ...
    if (proxy && typeof proxy === 'object') {
      ...
      try {
        ...
        logger.warn('Droid 代理配置解析失败，已忽略:', error.message)
        proxyConfig = null
      }
    }
    ...
    if (!isApiKeyProvision && normalizedRefreshToken && isManualProvision) {
      try {
        ...
        logger.info(
          ` [Droid manual] 刷新后令牌 - AccountName: ${name}, AccessToken: ${refreshed.accessToken || '[empty]'}, RefreshToken: ${refreshed.refreshToken || '[empty]'}, ExpiresAt: ${refreshed.expiresAt || '[empty]'}, ExpiresIn: ${
            refreshed.expiresIn !== null && refreshed.expiresIn !== undefined ? refreshed.expiresIn : '[empty]'
          }`,
        )
        ...
        normalizedAuthenticationMethod = refreshed.authenticationMethod || normalizedAuthenticationMethod
        ...
        if (refreshed.user) {
          ...
          if (derivedName) {
            ...
            normalizedOwnerDisplayName = normalizedOwnerDisplayName || normalizedOwnerEmail || normalizedOwnerName
          }
        ...
        throw new Error(`Refresh Token 验证失败：${error.message}`, { cause: error })
      }
      ...
      try {
        ...
        const selectedOrgId =
          ...
          (Array.isArray(orgIds) ? orgIds.find((id) => typeof id === 'string' && id.trim()) : null) ||
          ''

        if (!selectedOrgId) {
          logger.warn(`[Droid oauth] 未获取到组织ID，跳过 WorkOS 刷新: ${name} (${accountId})`)
        } else {
          const refreshed = await this._refreshTokensWithWorkOS(normalizedRefreshToken, proxyConfig, selectedOrgId)
        ...
        logger.warn(`[Droid oauth] 初始化刷新失败: ${name} (${accountId}) - ${error.message}`)
      }
    }
    ...
    logger.info(
      ` [Droid ${provisioningMode}] 写入前令牌快照 - AccountName: ${name}, AccessToken: ${normalizedAccessToken || '[empty]'}, RefreshToken: ${normalizedRefreshToken || '[empty]'}, ExpiresAt: ${normalizedExpiresAt || '[empty]'}, ExpiresIn: ${
        normalizedExpiresIn !== null && normalizedExpiresIn !== undefined ? normalizedExpiresIn : '[empty]'
      }`,
    )
    ...
    const accountData = {
      ...
      ownerDisplayName: normalizedOwnerDisplayName || normalizedOwnerName || normalizedOwnerEmail || '',
      userId: normalizedUserId || '',
      tokenType: normalizedTokenType || 'Bearer',
      authenticationMethod: normalizedAuthenticationMethod || '',
      expiresIn: normalizedExpiresIn !== null && normalizedExpiresIn !== undefined ? String(normalizedExpiresIn) : '',
    ...
    logger.success(`Created Droid account: ${name} (${accountId}) - Endpoint: ${normalizedEndpointType}`)
    ...
    try {
      ...
      logger.warn(`[Droid ${provisioningMode}] 写入后验证失败: ${name} (${accountId}) - ${verifyError.message}`)
    }
  ...
  async updateAccount(accountId, updates) {
    ...
    const hasStoredAccount = storedAccount && typeof storedAccount === 'object' && Object.keys(storedAccount).length > 0
    ...
    const parseProxyConfig = (value) => {
      ...
      if (typeof value === 'string' && value.trim()) {
        try {
          ...
          logger.warn('Failed to parse stored Droid proxy config:', error.message)
        }
      }
      return null
    }
    ...
    const hasNewRefreshToken = typeof sanitizedUpdates.refreshToken === 'string' && sanitizedUpdates.refreshToken

    if (hasNewRefreshToken) {
      try {
        const refreshed = await this._refreshTokensWithWorkOS(sanitizedUpdates.refreshToken, proxyConfig)
        ...
        sanitizedUpdates.expiresAt = refreshed.expiresAt || sanitizedUpdates.expiresAt || account.expiresAt || ''
        ...
        sanitizedUpdates.authenticationMethod = refreshed.authenticationMethod || account.authenticationMethod || ''
        sanitizedUpdates.organizationId =
          sanitizedUpdates.organizationId || refreshed.organizationId || account.organizationId || ''
        ...
        if (refreshed.user) {
          ...
          if (derivedName) {
            ...
            sanitizedUpdates.ownerDisplayName = sanitizedUpdates.ownerDisplayName || sanitizedUpdates.ownerEmail
        ...
        throw new Error(`Refresh Token 验证失败：${error.message || '未知错误'}`, { cause: error })
      }
    }
    ...
    const existingApiKeyEntries = this._parseApiKeyEntries(
      hasStoredAccount && Object.prototype.hasOwnProperty.call(storedAccount, 'apiKeys') ? storedAccount.apiKeys : '',
    )
    ...
    const rawApiKeyMode =
      typeof updates.apiKeyUpdateMode === 'string' ? updates.apiKeyUpdateMode.trim().toLowerCase() : ''

    let apiKeyUpdateMode = ['append', 'replace', 'delete', 'update'].includes(rawApiKeyMode) ? rawApiKeyMode : ''
    ...
    if (apiKeyUpdateMode === 'delete') {
      ...
      if (removalHashes.size > 0) {
        mergedApiKeys = existingApiKeyEntries.filter((entry) => entry && entry.hash && !removalHashes.has(entry.hash))
        removedCount = existingApiKeyEntries.length - mergedApiKeys.length
        apiKeysUpdated = removedCount > 0

        if (!apiKeysUpdated) {
          logger.warn(`删除模式未匹配任何 Droid API Key: ${accountId} (提供 ${removalHashes.size} 条)`)
        }
      } else if (removeApiKeysInput.length > 0) {
        logger.warn(`删除模式未收到有效的 Droid API Key: ${accountId}`)
      }
      ...
      for (const updateItem of newApiKeysInput) {
        ...
        if (existingIndex !== -1) {
          ...
          mergedApiKeys[existingIndex] = {
            ...
            usageCount:
              updateItem.usageCount !== undefined ? String(updateItem.usageCount) : existingEntry.usageCount || '0',
          }
          apiKeysUpdated = true
        }
      }
      ...
      if (!apiKeysUpdated) {
        logger.warn(`更新模式未匹配任何 Droid API Key: ${accountId} (提供 ${updatedHashes.size} 个哈希)`)
      }
    } else {
      ...
      mergedApiKeys = this._buildApiKeyEntries(newApiKeysInput, existingApiKeyEntries, clearExisting)
    ...
    const enablingAutoProtection =
      sanitizedUpdates.disableAutoProtection === true || sanitizedUpdates.disableAutoProtection === 'true'
    if (enablingAutoProtection) {
      ...
      if (hasErroredKey) {
        const resetEntries = mergedApiKeys.map((entry) =>
          entry && entry.status === 'error' ? { ...entry, status: 'active', errorMessage: '' } : entry,
        )
  ...
  async deleteAccount(accountId) {
    ...
    logger.success(`Deleted Droid account: ${accountId}`)
  }
  ...
  async refreshAccessToken(accountId, proxyConfig = null) {
    ...
    try {
      ...
      const refreshed = await this._refreshTokensWithWorkOS(account.refreshToken, proxy, account.organizationId || null)
      ...
      if (refreshed.user) {
        ...
        const updates = {}
        logger.info(`Droid token refreshed for: ${user.email} (${user.first_name} ${user.last_name})`)
      ...
      if (account?.disableAutoProtection === true || account?.disableAutoProtection === 'true') {
        logger.info(`Droid 账号 ${accountId} 已关闭自动防护，token 刷新失败时跳过写 error 状态`)
        upstreamErrorHelper
          .recordErrorHistory(
            accountId,
            'droid',
    ...
    return allAccounts
      .filter((account) => {
        ...
        if (this.isSubscriptionExpired(account)) {
          logger.debug(`Skipping expired Droid account: ${account.name}, expired at ${account.subscriptionExpiresAt}`)
          return false
        }
  ...
  async selectAccount(endpointType = null) {
    ...
    if (accounts.length === 0 && endpointType) {
      logger.warn(`No Droid accounts found for endpoint ${endpointType}, falling back to any available account`)
  ...
  async touchLastUsedAt(accountId) {
    ...
    try {
      ...
      logger.warn(`Failed to update lastUsedAt for Droid account ${accountId}:`, error)
    }
  }
...
// 导出单例
export const droidAccountService = new DroidAccountService()
