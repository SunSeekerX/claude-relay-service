...
const getDefaultRedirectUri = function getDefaultRedirectUri(oauthProvider) {
  ...
  return env.GEMINI_OAUTH_REDIRECT_URI || 'https://codeassist.google.com/authcode'
}
...
router.post(
  ...
  asyncRoute('Failed to generate Gemini auth URL', async (req) => {
    ...
    } = await geminiAccountService.generateAuthUrl(state, redirectUri, effectiveProxy, oauthProvider)
