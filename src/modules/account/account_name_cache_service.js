...
import { geminiApiAccountService as geminiApiAccountServiceMod } from './account_gemini_api_service.js'
import { openaiResponsesAccountService as openaiResponsesAccountServiceMod } from './account_openai_responses_service.js'
...
class AccountNameCacheService {
  ...
  async refresh() {
    ...
    try {
      ...
      // 可选服务（可能不存在）
      const geminiApiAccountService = geminiApiAccountServiceMod
      const openaiResponsesAccountService = openaiResponsesAccountServiceMod
      ...
      const bedrockResult = results[7].status === 'fulfilled' ? results[7].value : { success: false, data: [] }
      ...
      logger.debug(`账户名称缓存已刷新: ${newAccountCache.size} 个账户, ${newGroupCache.size} 个分组`)
    } catch (error) {
...
// 单例导出
export const accountNameCacheService = new AccountNameCacheService()
