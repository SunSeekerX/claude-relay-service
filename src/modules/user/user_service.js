...
let _apiKeyService = null
const getApiKeyService = () => {
  if (!_apiKeyService) {
    _apiKeyService = require('../apikey/apikey_service.js').apiKeyService
  }
...
class UserService {
  ...
  async calculateUserUsageStats(userId) {
    try {
      ...
      const userApiKeys = await getApiKeyService().getUserApiKeys(userId, true) // Include deleted keys for stats
  ...
  async transferMatchingApiKeys(user) {
    try {
      ...
      // 获取所有API Keys
      const allApiKeys = await getApiKeyService().getAllApiKeysFast()
      ...
      for (const apiKey of matchingKeys) {
        try {
          await getApiKeyService().updateApiKey(apiKey.id, {
            userId: user.id,
