...
router.get('/claude-code-headers', authenticateAdmin, async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to get Claude Code headers', message: error.message })
  }
})
...
router.delete('/claude-code-headers/:accountId', authenticateAdmin, async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to clear Claude Code headers', message: error.message })
  }
})
...
router.get('/models/pricing', authenticateAdmin, async (req, res) => {
  try {
    ...
    res.json({
      ...
      data: data || {},
      meta: {
        seedModelCount: pricingService.pricingData ? Object.keys(pricingService.pricingData).length : 0,
