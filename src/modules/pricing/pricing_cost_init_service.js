...
import { CostCalculator } from './pricing_cost_calculator.js'
...
class CostInitService {
  ...
  async needsInitialization() {
    try {
      ...
      do {
        const [newCursor, keys] = await client.scan(cursor, 'MATCH', RedisKeys.usage.costPattern, 'COUNT', 100)
        cursor = newCursor
        if (keys.length > 0) {
          hasCostData = true
          break
      ...
      do {
        const [newCursor, usageKeys] = await client.scan(cursor, 'MATCH', 'usage:*:model:daily:*:*', 'COUNT', 100)
        cursor = newCursor
...
export const costInitService = new CostInitService()
