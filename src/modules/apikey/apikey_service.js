...
import { billingEventPublisher } from '../payment/payment_billing_event_publisher.js'
...
const getUserService = () => {
  if (!_userService) {
    _userService = require('../user/user_service.js').userService
  }
  return _userService
}
...
class ApiKeyService {
  ...
  async generateApiKey(options = {}) {
    ...
    return {
      ...
      enableOpenAIResponsesCodexAdaptation: parseBooleanWithDefault(keyData.enableOpenAIResponsesCodexAdaptation, true),
      enableOpenAIResponsesPayloadRules: parseBooleanWithDefault(keyData.enableOpenAIResponsesPayloadRules, false),
  ...
  async validateApiKey(apiKey) {
    try {
      ...
      if (keyData.userId) {
        try {
          const user = await getUserService().getUserById(keyData.userId, false)
      ...
      if (weeklyOpusCostLimit > 0) {
        ...
        costQueries.push(
          redis.getWeeklyOpusCost(keyData.id, resetDay, resetHour).then((v) => ({ weeklyOpusCost: v || 0 })),
        )
      }

      const costData = costQueries.length > 0 ? Object.assign({}, ...(await Promise.all(costQueries))) : {}
      ...
      const openaiResponsesPayloadRules = parseOpenAIResponsesPayloadRules(keyData.openaiResponsesPayloadRules)
  ...
  async validateApiKeyForStats(apiKey) {
    try {
      ...
      // 获取当日费用
      const [dailyCost, costStats] = await Promise.all([redis.getDailyCost(keyData.id), redis.getCostStats(keyData.id)])
  ...
  async getAllTags() {
    ...
    const [indexTags, manualTags] = await Promise.all([redis.scanAllApiKeyTags(), redis.getManualTags()])
  ...
  async renameTag(oldName, newName) {
    ...
    const manualTags = await redis.getManualTags()
    const foundInManual = manualTags.some((t) => typeof t === 'string' && t.trim() === normalizedOld)
  ...
  async getAllApiKeys(includeDeleted = false) {
    try {
      ...
      for (const key of apiKeys) {
        ...
        key.enableOpenAIResponsesPayloadRules = parseBooleanWithDefault(key.enableOpenAIResponsesPayloadRules, false)
        ...
        key.openaiResponsesPayloadRules = parseOpenAIResponsesPayloadRules(key.openaiResponsesPayloadRules)
        // 不暴露已弃用字段
        ...
        if (lastUsageRecord && (lastUsageRecord.accountId || lastUsageRecord.accountType)) {
          const resolvedAccount = await this._resolveLastUsageAccount(key, lastUsageRecord, accountInfoCache, client)
  ...
  async _enrichApiKeysWithStats(apiKeys) {
    try {
      ...
      for (const key of apiKeys) {
        ...
        let cacheCreateTokens = parseInt(usageTotal.totalCacheCreateTokens || usageTotal.cacheCreateTokens) || 0
        let cacheReadTokens = parseInt(usageTotal.totalCacheReadTokens || usageTotal.cacheReadTokens) || 0
        let ephemeral5mTokens = parseInt(usageTotal.totalEphemeral5mTokens || usageTotal.ephemeral5mTokens) || 0
        let ephemeral1hTokens = parseInt(usageTotal.totalEphemeral1hTokens || usageTotal.ephemeral1hTokens) || 0
        ...
        key.enableModelRestriction = key.enableModelRestriction === 'true' || key.enableModelRestriction === true
        key.enableClientRestriction = key.enableClientRestriction === 'true' || key.enableClientRestriction === true
        key.enableOpenAIResponsesCodexAdaptation = parseBooleanWithDefault(
          key.enableOpenAIResponsesCodexAdaptation,
          true,
        )
  ...
  async deleteApiKey(keyId, deletedBy = 'system', deletedByType = 'system') {
    try {
      ...
      multi.hset(redisKey, updatedData)
      multi.expire(redisKey, 86400 * 365)
      multi.zadd(apiKeyIndexService.INDEX_KEYS.DELETED_AT, new Date(updatedData.deletedAt).getTime(), keyId)
  ...
  async recordUsage(
    ...
    if (inputTokens && typeof inputTokens === 'object' && !Array.isArray(inputTokens)) {
      ...
      cacheCreateTokens = usageObject.cache_creation_input_tokens || usageObject.cache_creation_tokens || 0
      cacheReadTokens = usageObject.cache_read_input_tokens || usageObject.cache_read_tokens || 0
      usageObject._thinkingTokens = extractThinkingTokens(usageObject)
    ...
    try {
      ...
      // 思考 tokens：展示单独计；计费/allTokens 按「是否已含在 output」处理（见 isThinkingAlreadyInOutput）
      const thinkingTokens =
        (usageExtras && usageExtras._thinkingTokens) || extractThinkingTokens(usageExtras || {}) || 0
      const thinkingAlreadyInOutput = thinkingTokens > 0 && isThinkingAlreadyInOutput(usageExtras || {})
      const billableOutputTokens = thinkingAlreadyInOutput ? outputTokens : outputTokens + thinkingTokens
      const totalTokens = inputTokens + billableOutputTokens + cacheCreateTokens + cacheReadTokens
      ...
      const costInfo = CostCalculator.calculateCost(
        {
          ...
          // 计费口径：思考按输出价计入（Gemini thoughts / 未含在 output 的 reasoning）
      ...
      await redis.incrementTokenUsage(
        ...
        ratedCost,
        thinkingTokens,
        !thinkingAlreadyInOutput, // thinking 是否计入 allTokens（OpenAI 子集则 false）
      )
  ...
  async recordUsageWithDetails(
    ...
    try {
      ...
      const thinkingTokens = extractThinkingTokens(usageObject)
      const thinkingAlreadyInOutput = thinkingTokens > 0 && isThinkingAlreadyInOutput(usageObject || {})
      const billableOutputTokens = thinkingAlreadyInOutput ? outputTokens : outputTokens + thinkingTokens
      const totalTokens = inputTokens + billableOutputTokens + cacheCreateTokens + cacheReadTokens
      ...
      try {
        ...
        costInfo = {
          ...
          cacheCreateCost: Number(costs.cacheCreate ?? costs.cacheWrite ?? calculatedCost?.cacheCreateCost ?? 0) || 0,
          ...
          isLongContextRequest:
            calculatedCost?.isLongContextRequest === true || calculatedCost?.debug?.isLongContextRequest === true,
  ...
  async _validateKeyUsableStatus(keyData) {
    ...
    if (keyData.isActivated === 'true' && keyData.expiresAt && new Date() > new Date(keyData.expiresAt)) {
      const keyName = keyData.name || 'Unknown'
  ...
  async getChangeHistory(keyId, { page = 1, pageSize = 20 } = {}) {
    ...
    const [total, list] = await Promise.all([client.llen(redisKey), client.lrange(redisKey, start, end)])
