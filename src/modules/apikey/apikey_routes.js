...
import { apiKeyService } from './apikey_service.js'
...
import { accountNameCacheService } from '../account/account_name_cache_service.js'
...
router.get(
  ...
  asyncRoute('Failed to get API keys', async (req) => {
    ...
    // 验证排序参数（新增 cost 排序）
    const validSortFields = ['name', 'createdAt', 'expiresAt', 'lastUsedAt', 'isActive', 'status', 'cost']
    ...
    if (validSortBy === 'cost') {
      ...
      const effectiveCostTimeRange = validCostTimeRanges.includes(costTimeRange) ? costTimeRange : '7days'
    ...
    if (userIdsToFetch.length > 0) {
      // 批量获取用户信息
      const users = await Promise.all(userIdsToFetch.map((id) => userService.getUserById(id, false).catch(() => null)))
    ...
    for (const apiKey of result.items) {
      if (apiKey.userId && userMap.has(apiKey.userId)) {
        ...
        apiKey.ownerDisplayName = apiKey.createdBy === 'admin' ? 'Admin' : apiKey.createdBy || 'Admin'
      }
...
const getApiKeysSortedByCostCustom = async function getApiKeysSortedByCostCustom(options) {
  ...
  // 2. 转换为数组并排序
  const sortedEntries = [...costs.entries()].sort((a, b) => (sortOrder === 'desc' ? b[1] - a[1] : a[1] - b[1]))
  ...
  asyncRoute('Failed to get account binding counts', async (_req) => {
    ...
    for (const key of apiKeys) {
      ...
      if (key.claudeConsoleAccountId) {
        ...
        bindingCounts.claudeConsoleAccountId[id] = (bindingCounts.claudeConsoleAccountId[id] || 0) + 1
      }
...
const calculateKeyStats = async function calculateKeyStats(keyId, timeRange, startDate, endDate) {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    const inputTokenCount = parseInt(data.totalInputTokens) || parseInt(data.inputTokens) || 0
    const outputTokenCount = parseInt(data.totalOutputTokens) || parseInt(data.outputTokens) || 0
    const cacheCreateTokenCount = parseInt(data.totalCacheCreateTokens) || parseInt(data.cacheCreateTokens) || 0
    const cacheReadTokenCount = parseInt(data.totalCacheReadTokens) || parseInt(data.cacheReadTokens) || 0
    const baseTokenCount = inputTokenCount + outputTokenCount + cacheCreateTokenCount + cacheReadTokenCount
    // alltime 的历史键可能带有旧实现写入的部分 allTokens，不能采用；由分项与 thinkingInAllTokens 统一推导。
    const storedAllTokens = isAlltimeQuery ? Number.NaN : parseInt(data.allTokens)
    stats.allTokens += Number.isFinite(storedAllTokens)
      ? storedAllTokens
      : baseTokenCount + (isAlltimeQuery ? parseInt(data.thinkingInAllTokens) || 0 : 0)
    stats.ephemeral5mTokens += parseInt(data.totalEphemeral5mTokens) || parseInt(data.ephemeral5mTokens) || 0
    stats.ephemeral1hTokens += parseInt(data.totalEphemeral1hTokens) || parseInt(data.ephemeral1hTokens) || 0
...
router.post(
  ...
  asyncRoute('Failed to create API key', async (req) => {
    ...
    const newKey = await apiKeyService.generateApiKey({
      ...
      serviceRates,
      weeklyResetDay:
        weeklyResetDay !== undefined && weeklyResetDay !== null && weeklyResetDay !== '' ? Number(weeklyResetDay) : 1,
      weeklyResetHour:
        weeklyResetHour !== undefined && weeklyResetHour !== null && weeklyResetHour !== ''
          ? Number(weeklyResetHour)
          : 0,
      enableOpenAIResponsesCodexAdaptation:
        enableOpenAIResponsesCodexAdaptation !== undefined ? enableOpenAIResponsesCodexAdaptation : true,
      enableOpenAIResponsesPayloadRules:
  ...
  asyncRoute('Failed to batch edit API keys', async (req) => {
    ...
    for (const keyId of keyIds) {
      try {
        ...
        // 重置配置变更后触发单 Key 回填
        if (finalUpdates.weeklyResetDay !== undefined || finalUpdates.weeklyResetHour !== undefined) {
          setImmediate(async () => {
            try {
