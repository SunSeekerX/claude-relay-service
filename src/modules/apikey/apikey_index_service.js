...
class ApiKeyIndexService {
  constructor() {
    ...
    this.DRIFT_SCAN_INTERVAL_MS = parseInt(env.APIKEY_INDEX_DRIFT_SCAN_INTERVAL_MS, 10) || 30 * 60 * 1000
  ...
  async rebuildIndexes() {
    ...
    try {
      ...
      let cursor = '0'
      do {
        const [newCursor, keys] = await client.scan(cursor, 'MATCH', RedisKeys.apiKey.tagPattern, 'COUNT', 100)
        cursor = newCursor
        if (keys.length > 0) {
      ...
      for (let i = 0; i < keyIds.length; i += BATCH_SIZE) {
        ...
        for (const apiKey of apiKeys) {
          ...
          const { keyId, createdScore, lastUsedScore, nameMember, deletedScore, isActive, isDeleted } =
  ...
  async detectAndHealMainIndexDrift() {
    ...
    const [allMembers, activeMembers, deletedMembers, createdFlat, lastUsedFlat, nameMembers, deletedAtFlat, keyIds] =
      await Promise.all([
    ...
    if (!reason) {
      ...
      for (let i = 0; i < keyIds.length && !reason; i += BATCH) {
        ...
        for (const apiKey of apiKeys) {
          ...
          const { keyId, createdScore, lastUsedScore, deletedScore, nameMember, isActive, isDeleted } =
            this._indexEntryForKey(apiKey)
          if (!isDeleted) {
    ...
    if (!reason) {
      ...
      let cursor = '0'
      do {
        const [next, tagKeys] = await client.scan(cursor, 'MATCH', RedisKeys.apiKey.tagPattern, 'COUNT', 200)
        cursor = next
        for (const tagKey of tagKeys) {
          actual.set(tagKey.slice(RedisKeys.apiKey.tag('').length), new Set(await client.smembers(tagKey)))
        }
      } while (cursor !== '0')
  ...
  async queryDeletedWithIndex({ page = 1, pageSize = 20 } = {}) {
    ...
    const pageKeyIds = total > 0 ? await client.zrevrange(this.INDEX_KEYS.DELETED_AT, start, end) : []
...
// 单例
export const apiKeyIndexService = new ApiKeyIndexService()
