...
const GROK_MEDIA_FALLBACK_PRICING = {
  'grok-imagine-image': {
...
const resolveImageResolution = (size) => {
  ...
  if (matched) {
    ...
    if (Number.isFinite(maxEdge) && maxEdge >= 1536) return '2k'
    return '1k'
  }
  if (text.includes('2k') || text.includes('2048') || text.includes('1920')) return '2k'
  if (text.includes('1k') || text.includes('1024') || text.includes('512')) return '1k'
  return '1k'
}

// low | medium（2.0 质量档）；其它/空 → low
const resolveImageQuality = (quality) => {
  const text = String(quality || '')
    .trim()
    .toLowerCase()
  if (!text) return 'low'
  if (text.includes('med') || text.includes('high') || text === 'hd' || text === '2') return 'medium'
  return 'low'
}

// 480p | 720p | 1080p
const resolveVideoResolution = (resolution, size) => {
  ...
  if (matched) {
    ...
    if (minEdge >= 1000) return '1080p'
    if (minEdge >= 700) return '720p'
    return '480p'
  }
  return '480p'
}
...
const resolveGrokMediaUnitPrices = (pricing, usage = {}) => {
  const base = {
  ...
  if (!pricing || typeof pricing !== 'object') return base
  ...
  if (imageTiers && typeof imageTiers === 'object') {
    ...
    if (imageTiers[compound] != null) {
      ...
      const tierValues = Object.values(imageTiers).map(num).filter((v) => v > 0)
  ...
  if (videoTiers && typeof videoTiers === 'object') {
    const res = resolveVideoResolution(
      usage.video_resolution || usage.resolution,
    ...
    if (videoTiers[res] != null) {
      ...
      if (res === '1080p' || res === '720p') {
        const tierValues = Object.values(videoTiers).map(num).filter((v) => v > 0)
        if (tierValues.length) base.videoOutputPerSecond = Math.max(...tierValues)
