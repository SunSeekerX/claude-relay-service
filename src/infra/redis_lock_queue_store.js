...
export const attach = function attach(redisClient) {
  ...
  redisClient.releaseAccountLock = async function (lockKey, lockValue) {
    try {
      ...
      const result = await this.client.eval(RedisLua.lock.compareAndDel, 1, lockKey, lockValue)
      return result === 1
    } catch (error) {
  ...
  redisClient.acquireUserMessageLock = async function (accountId, requestId, lockTtlMs, delayMs) {
    ...
    try {
      const result = await this.client.eval(
        RedisLua.lock.acquireUserMessage,
        2,
        lockKey,
        lastTimeKey,
        requestId,
  ...
  redisClient.releaseUserMessageLock = async function (accountId, requestId) {
    ...
    try {
      const result = await this.client.eval(RedisLua.lock.releaseUserMessage, 2, lockKey, lastTimeKey, requestId)
      return result === 1
    } catch (error) {
