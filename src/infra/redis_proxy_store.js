...
export const attach = function attach(redisClient) {
  ...
  redisClient.pushProxyHealthHistory = async function (proxyId, entry, maxLen = LIMITS.proxyHealthHistory) {
    const client = this.getClientSafe()
  ...
  redisClient.setProxyQualityResult = async function (proxyId, result) {
    const client = this.getClientSafe()
    await client.set(RedisKeys.proxy.qualityResult(proxyId), JSON.stringify(result), 'EX', TTL.proxyQuality)
  }
  ...
  redisClient.setProxyExitInfo = async function (proxyId, info) {
    const client = this.getClientSafe()
    await client.set(RedisKeys.proxy.exitInfo(proxyId), JSON.stringify(info), 'EX', TTL.proxyExitInfo)
  }
  ...
  redisClient.clearProxyBindingFromAllAccounts = async function (kind, id) {
    ...
    for (const store of PROXY_BINDABLE_STORES) {
      ...
      const accountIds = await this.getAllIdsByIndex(store.indexKey, `${store.prefix}*`, extractRegex)
