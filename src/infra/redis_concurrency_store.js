...
export const attach = function attach(redisClient) {
  ...
  redisClient._getConcurrencyConfig = function () {
    ...
    return {
      ...
      renewIntervalSeconds: normalizeNumber(configValues.renewIntervalSeconds, defaults.renewIntervalSeconds, {
        allowZero: true,
        min: 0,
      }),
      cleanupGraceSeconds: normalizeNumber(configValues.cleanupGraceSeconds, defaults.cleanupGraceSeconds, {
        min: 0,
      }),
    }
  }
  ...
  redisClient.incrConcurrency = async function (apiKeyId, requestId, leaseSeconds = null) {
    ...
    try {
      const { leaseSeconds: defaultLeaseSeconds, cleanupGraceSeconds } = this._getConcurrencyConfig()
  ...
  // 刷新 Console 账户并发租约
  redisClient.refreshConsoleAccountConcurrencyLease = async function (accountId, requestId, leaseSeconds = null) {
    if (!requestId) {
      return 0
    }
    const compositeKey = RedisKeys.concurrency.consoleAccount(accountId)
  ...
  // OpenAI-Responses 账户并发（复用 ZSet 租约机制）
  redisClient.incrOpenaiResponsesAccountConcurrency = async function (accountId, requestId, leaseSeconds = null) {
    if (!requestId) {
      throw new Error('Request ID is required for openai-responses account concurrency tracking')
    }
    const compositeKey = RedisKeys.concurrency.openaiResponsesAccount(accountId)
    return await this.incrConcurrency(compositeKey, requestId, leaseSeconds)
  }

  redisClient.refreshOpenaiResponsesAccountConcurrencyLease = async function (
    accountId,
    requestId,
    leaseSeconds = null,
  ) {
  ...
  redisClient.decrOpenaiResponsesAccountConcurrency = async function (accountId, requestId) {
    const compositeKey = RedisKeys.concurrency.openaiResponsesAccount(accountId)
    return await this.decrConcurrency(compositeKey, requestId)
  }

  redisClient.getOpenaiResponsesAccountConcurrency = async function (accountId) {
    const compositeKey = RedisKeys.concurrency.openaiResponsesAccount(accountId)
    return await this.getConcurrency(compositeKey)
  }
