...
export const attach = function attach(redisClient) {
  ...
  redisClient.migrateUsageIndex = async function () {
    ...
    try {
      ...
      let cursor = '0'
      do {
        const [newCursor, keys] = await this.client.scan(cursor, 'MATCH', RedisKeys.usage.dailyPattern, 'COUNT', 500)
        cursor = newCursor
      ...
      cursor = '0'
      do {
        const [newCursor, keys] = await this.client.scan(cursor, 'MATCH', RedisKeys.usage.hourlyPattern, 'COUNT', 500)
        cursor = newCursor
        const pipeline = this.client.pipeline()
        for (const key of keys) {
