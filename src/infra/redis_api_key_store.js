...
import { resolveKeyIdByHash } from '../common/compat_api_key_hash.js'

// 懒加载业务 service，打断 ESM 环：redis → api_key_store → accountNameCache/account services → redis
const require = createRequire(import.meta.url)
let _apiKeyIndexService = null
let _accountNameCacheService = null
const getApiKeyIndexService = () => {
  if (!_apiKeyIndexService) {
    _apiKeyIndexService = require('../modules/apikey/apikey_index_service.js').apiKeyIndexService
  }
  return _apiKeyIndexService
}
const getAccountNameCacheService = () => {
  if (!_accountNameCacheService) {
    _accountNameCacheService = require('../modules/account/account_name_cache_service.js').accountNameCacheService
  }
  return _accountNameCacheService
}
...
export const attach = function attach(redisClient) {
  ...
  redisClient._checkIndexReady = async function () {
    try {
      ...
      // 必须与 getApiKeyIndexService().CURRENT_VERSION 保持一致（升级版本时两处都要改）。
  ...
  redisClient._scanTagsFallback = async function () {
    ...
    do {
      ...
      const validKeys = keys.filter((k) => k !== RedisKeys.apiKey.hashMap && k.split(':').length === 2)
  ...
  redisClient._parseApiKeyData = function (data) {
    ...
    // 数组字段（JSON 解析）
    const arrayFields = ['tags', 'restrictedModels', 'allowedClients', 'openaiResponsesPayloadRules']
  ...
  redisClient.getApiKeysPaginated = async function (options = {}) {
    ...
    // 尝试使用索引查询（性能优化）
    const indexReady = await getApiKeyIndexService().isIndexReady()
    ...
    if (canUseIndex) {
      // 使用索引查询
      try {
        return await getApiKeyIndexService().queryWithIndex({
          page,
          pageSize,
          sortBy,
          sortOrder,
    ...
    if (search) {
      ...
      if (searchMode === 'apiKey') {
        ...
        filteredKeys = getAccountNameCacheService().searchByBindingAccount(filteredKeys, lowerSearch)
      }
    }
