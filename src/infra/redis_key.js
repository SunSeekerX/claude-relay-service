...
const getConfig = () => {
  if (!_config) {
    try {
      _config = require('../../config/config.js').config
    } catch {
      _config = {}
    }
  }
...
export const RedisKeys = {
  ...
  accountUsage: {
    ...
    modelDaily: (accountId, model, date) => `account_usage:model:daily:${accountId}:${model}:${date}`,
    modelMonthly: (accountId, model, month) => `account_usage:model:monthly:${accountId}:${model}:${month}`,
    modelHourly: (accountId, model, hour) => `account_usage:model:hourly:${accountId}:${model}:${hour}`,
  ...
  accounts: {
    ...
    droidApiKey: (accountId, endpoint, sessionHash) => `droid_api_key:${accountId}:${endpoint}:${sessionHash}`,
    // Grok / xAI: 冒号
  ...
  session: {
    ...
    // Codex Realtime call_id → 建连账户（sideband WS 复用同一 OAuth 号）
    codexRealtimeCall: (callId) => `codex_realtime_call:${callId}`,
...
export const TTL = {
  ...
  // Realtime call 绑定：覆盖一次语音会话侧带连接窗口
  codexRealtimeCall: 2 * 60 * 60,
  ...
  upstream503: () => {
    ...
    const envValue = parseInt(env.UPSTREAM_ERROR_503_TTL_SECONDS, 10)
    return (
  ...
  upstreamMaxCustom: () => {
    const config = getConfig()
    const envValue = parseInt(env.UPSTREAM_ERROR_MAX_CUSTOM_TTL_SECONDS, 10)
    return (
      config.upstreamError?.maxCustomTtlSeconds ?? (Number.isFinite(envValue) && envValue > 0 ? envValue : null) ?? 1800
    )
  },
  errorHistory: () => {
  ...
  // 并发租约(参数派生,毫秒)
  concurrencyLeaseMs: (leaseSeconds, graceSeconds) => Math.max((leaseSeconds + graceSeconds) * 1000, 60000),
}
