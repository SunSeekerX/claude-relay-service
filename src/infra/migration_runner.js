...
import { getAppVersion, versionGt } from '../common/common_helper.js'
import * as ledger from './migration_ledger.js'
import { registry } from './migration_registry.js'

// 首次接管:把存量等价旧 marker 平滑导入 applied 台账(只读旧 marker,绝不删)。
// per-migration 判断(不用全局 guard):将来新增带 legacyMarkerKey 的迁移,其存量旧 marker 仍会被接管。
export const importLegacyMarkers = async (client) => {
...
export const runVersionGated = async (redis) => {
...
export const runMarker = async (redis, id) => {
