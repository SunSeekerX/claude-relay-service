...
const IMPORTABLE_MODES = new Set([
  'chat',
  'responses',
  'completion',
  'embedding',
...
const extractTiersPerM = (raw, suffix = '') => {
  ...
  for (const key of Object.keys(raw || {})) {
    const matched =
      key.match(/_per_token_above_(\d+)k_tokens(_.+)?$/) ||
      key.match(/token_cost_above_(\d+)k_tokens(_.+)?$/)
    if (!matched) {
      continue
    }
      ...
      return {
        ...
        cache_write_1h_per_million_tokens: toPerMOpt(
          getFirstDefined(
            raw,
            suffixes.map(
              (sfx) => `cache_creation_input_token_cost_above_1hr_above_${tag}_tokens${sfx}`
            )
          )
        ),
...
const derivePriorityTiersFromBase = (baseTiers, basePrices, priorityPrices) => {
  ...
  const derived = baseTiers.map((tier) => ({
    ...
    input_per_million_tokens: scalePerMField(
      tier.input_per_million_tokens,
      basePrices.input,
      priorityPrices.input
    ),
    output_per_million_tokens: scalePerMField(
      tier.output_per_million_tokens,
      basePrices.output,
      priorityPrices.output
    ),
...
const buildMultimodalPricing = (raw) => {
  ...
  const videoInputTiers = [8, 15]
    .map((seconds) => {
      ...
      return value != null
        ? { threshold_seconds: seconds, input_per_second: toStr(value) }
        : null
    })
    .filter(Boolean)
  ...
  return {
    ...
    audio_input_per_second: toStr(
      raw.input_cost_per_audio_per_second ?? raw.input_cost_per_second
    ),
    audio_output_per_second: toStr(
      raw.output_cost_per_audio_per_second ?? raw.output_cost_per_second
    ),
  ...
  const pricing = {
    ...
    flex_pricing: hasFlex
      ? {
          ...
          cache_write_1h_per_million_tokens: toTier1hPerMOpt(
            flexCacheWrite1h,
            flexPricingRaw.cacheWrite
          ),
    ...
    fast_rate_multiplier:
      typeof source.provider_specific_entry?.fast === 'number' &&
      source.provider_specific_entry.fast > 0
...
const metadataNonCostBase = (metadata) => {
  ...
  for (const [key, value] of Object.entries(metadata)) {
    if (isLiteLLMCostKey(key)) continue
    base[key] = value
  }
  return base
}
...
const internalToLiteLLM = (record) => {
  ...
  setIfDefined(
    ...
    perMToToken(pricing.cache_write_per_million_tokens)
  )
  setIfDefined(
    out,
    'cache_creation_input_token_cost_above_1hr',
    perMToToken(pricing.cache_write_1h_per_million_tokens)
  )
  setIfDefined(
    out,
    'cache_read_input_token_cost',
    perMToToken(pricing.cache_read_per_million_tokens)
  )
  setIfDefined(
    out,
    'output_cost_per_reasoning_token',
    perMToToken(pricing.reasoning_per_million_tokens)
  )
  ...
  if (pricing.priority_pricing) {
    ...
    setIfDefined(
      ...
      perMToToken(pp.cache_write_per_million_tokens)
    )
    setIfDefined(
      out,
      'cache_creation_input_token_cost_above_1hr_priority',
    ...
    setIfDefined(
      ...
      perMToToken(pp.cache_read_per_million_tokens)
    )
  ...
  if (pricing.flex_pricing) {
    ...
    setIfDefined(
      out,
      'cache_creation_input_token_cost_flex',
      perMToToken(fp.cache_write_per_million_tokens)
    )
    setIfDefined(
      out,
      'cache_creation_input_token_cost_above_1hr_flex',
    ...
    setIfDefined(
      out,
      'cache_read_input_token_cost_flex',
      perMToToken(fp.cache_read_per_million_tokens)
    )
  ...
  setIfDefined(
    ...
    perMToToken(pricing.audio_output_per_million_tokens)
  )
  ...
  setIfDefined(
    ...
    perMToToken(pricing.audio_cache_write_per_million_tokens)
  )
  setIfDefined(
    out,
    'cache_read_input_audio_token_cost',
    perMToToken(pricing.audio_cache_read_per_million_tokens)
  )
  ...
  setIfDefined(
    ...
    perMToToken(pricing.image_input_per_million_tokens)
  )
  setIfDefined(
    out,
    'output_cost_per_image_token',
    perMToToken(pricing.image_output_per_million_tokens)
  )
  ...
  setIfDefined(
    ...
    perMToToken(pricing.image_cache_read_per_million_tokens)
  )
  ...
  setIfDefined(
    ...
    perMToToken(pricing.cache_hit_per_million_tokens)
  )
  setIfDefined(
    out,
  ...
  if (pricing.search_context_cost) {
    out.search_context_cost_per_query = {
      search_context_size_low:
        pricing.search_context_cost.low != null
          ? Number(pricing.search_context_cost.low)
          : undefined,
      search_context_size_medium:
        pricing.search_context_cost.medium != null
          ? Number(pricing.search_context_cost.medium)
          : undefined,
      search_context_size_high:
        pricing.search_context_cost.high != null
          ? Number(pricing.search_context_cost.high)
          : undefined
    }
  }
  ...
  if (Array.isArray(pricing.image_output_tiers)) {
    ...
    for (const tier of pricing.image_output_tiers) {
      ...
      if (!field || tier.output_per_image == null || tier.output_per_image === '') continue
      const num = Number(tier.output_per_image)
      if (Number.isFinite(num)) out[field] = num
    }
  }
  ...
  if (Array.isArray(pricing.video_input_tiers)) {
    for (const tier of pricing.video_input_tiers) {
      ...
      if (!Number.isFinite(seconds) || tier.input_per_second == null || tier.input_per_second === '') continue
...
const isFiniteNonNegNumber = (value) => {
...
const hasAnyPriceValue = (value, keyHint = '') => {
  if (value == null || value === '') return false
  if (NON_PRICE_KEYS.has(keyHint)) return false
  if (Array.isArray(value)) return value.some((item) => hasAnyPriceValue(item))
...
const hasBillingPricing = (record) => {
  if (!record || typeof record !== 'object') return false
  if (record.hasBilling === false) return false
...
const assertAndSanitizePricing = (pricing) => {
    ...
    if (Array.isArray(obj)) {
      return obj
        ...
        .filter((item) => {
          ...
          return hasAnyPriceValue(item) || keys.some((key) => !NON_PRICE_KEYS.has(key) && item[key] != null && item[key] !== '')
        })
    }
    for (const [key, value] of Object.entries(obj)) {
      if (Array.isArray(value) && (key.endsWith('_tiers') || key === 'price_tiers')) {
        const next = pruneEmptyTiers(value)
        if (!next.length) delete obj[key]
        else obj[key] = next
...
const sanitizeMetadataValue = (value, depth, keyPath) => {
  ...
  if (typeof value === 'object') {
    ...
    for (const [key, child] of Object.entries(value)) {
      if (METADATA_SENSITIVE_KEY.test(key)) continue
...
const sanitizeMetadataForStore = (metadata) => {
  ...
  try {
    ...
    if (error && error.message && error.message.includes('metadata')) throw error
    return null
  }
  return cleaned
}
...
const isPublicPricingKey = (key) => {
  const k = String(key || '')
  if (PUBLIC_PRICING_KEYS.has(k)) return true
  // 价格相关字段全部放行（计费展示需要）
  if (k.includes('cost') || k.includes('price') || k.startsWith('input_') || k.startsWith('output_')) {
    return true
  }
  if (k.startsWith('cache_') || k.startsWith('search_context')) return true
  if (k === 'provider_specific_entry' || k.startsWith('xai_')) return true
  return false
}

const toPublicPricingEntry = (entry) => {
  if (!entry || typeof entry !== 'object') return entry
  const out = {}
  for (const [key, value] of Object.entries(entry)) {
    if (!isPublicPricingKey(key)) continue
    out[key] = value
  }
  return out
}

const toPublicPricingMap = (pricingMap) => {
