<template>
  <div class="flex min-h-0 flex-1 flex-col">
    ...
    <!-- Tab 切换 + 全局配置入口 -->
    <SegmentedTabs v-model="activeTab" class="mb-4" :tabs="tabs" variant="underline">
      <template #extra>
        <button
          class="seg-tabs-extra-btn px-4 py-2 text-sm font-medium transition-colors"
          type="button"
          @click="openConfigModal"
        >
          <i class="i-lucide-settings mr-1" />全局配置
        </button>
      </template>
    </SegmentedTabs>
    ...
    <ModalTransition>
      <div v-if="configModal" class="modal fixed inset-0 z-50 flex items-center justify-center p-4">
        <div class="modal-content mx-auto max-h-[90vh] w-full max-w-2xl overflow-y-auto p-3 sm:p-4">
          ...
          <div class="mt-6 flex gap-3">
            ...
            <button
              ...
              @click="saveConfig"
            >
              <i v-if="savingConfig" class="i-lucide-loader-circle animate-spin mr-2" />保存配置
            </button>
          </div>
        </div>
      </div>
    ...
    <div v-show="activeTab === 'plans'" class="space-y-3">
      <div class="flex justify-end">
        <button :class="BTN_PRIMARY" @click="openPlanModal()">
          <i class="i-lucide-plus mr-1" />新建商品
        </button>
      </div>
    ...
    <div v-show="activeTab === 'providers'" class="space-y-3">
      <div class="flex justify-end">
        <button :class="BTN_PRIMARY" @click="openProviderModal()">
          <i class="i-lucide-plus mr-1" />新建渠道
        </button>
      </div>
      <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
    ...
    <div v-show="activeTab === 'orders'" class="space-y-3">
      <div class="flex items-center gap-2">
        ...
        <button :class="BTN_SECONDARY" @click="reloadOrders">
          <i class="i-lucide-refresh-cw mr-1" />刷新
        </button>
      </div>
      <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
    ...
    <div v-show="activeTab === 'dashboard'" class="space-y-4">
      <div class="flex items-center justify-end">
        <button :class="BTN_SECONDARY" :disabled="dashboardLoading" @click="loadDashboard">
          <i :class="['i-lucide-redo-2 mr-1', dashboardLoading && 'animate-spin']" />刷新
        </button>
      </div>
    ...
    <ModalTransition>
      <div
        ...
        class="modal fixed inset-0 z-50 flex items-center justify-center p-4"
      >
        <div class="modal-content mx-auto w-full max-w-md p-3 sm:p-4">
    ...
    <ModalTransition>
      <div
        ...
        class="modal fixed inset-0 z-50 flex items-center justify-center p-4"
      >
        <div class="modal-content mx-auto w-full max-w-sm p-3 sm:p-4">
...
const TD_EMPTY = 'px-4 py-5 text-center text-sm text-gray-400'
const INPUT =
  'form-input w-full'
...
const STAT_CARD =
  'flex items-center justify-between gap-3 rounded-xl bg-[var(--surface-muted)] px-3 py-2.5 dark:bg-gray-800/80'
const STAT_LABEL = 'text-sm text-gray-500 dark:text-gray-400'
const STAT_VALUE = 'text-base font-bold tabular-nums text-gray-800 dark:text-gray-100'

const tabs = [
  { key: 'dashboard', name: '数据看板', icon: 'i-lucide-chart-pie' },
  { key: 'plans', name: '充值商品', icon: 'i-lucide-box' },
  { key: 'providers', name: '支付渠道', icon: 'i-lucide-plug' },
  { key: 'orders', name: '订单管理', icon: 'i-lucide-list' }
]
