    ...
    <div v-if="mode === 'compact'" class="flex items-center gap-2">
      ...
      <button class="theme-toggle-button" :title="themeTooltip" @click="handleCycleTheme">
        <transition mode="out-in" name="fade">
          <i v-if="themeStore.themeMode === 'light'" key="sun" class="i-lucide-sun" />
          <i v-else-if="themeStore.themeMode === 'dark'" key="moon" class="i-lucide-moon" />
          <i v-else key="auto" class="i-lucide-contrast" />
        </transition>
      </button>
    </div>
    ...
    <div v-else-if="mode === 'dropdown'" class="theme-switch-wrapper">
      ...
      <div class="relative mr-3">
        ...
        >
          ...
          />
          <i class="i-lucide-palette ml-1 text-sm opacity-60" />
        </button>
        <!-- 色系下拉菜单 -->
        <transition name="dropdown">
          <div v-if="showColorMenu" class="color-menu">
      ...
      >
        ...
        <div class="switch-handle">
          <div class="handle-icon">
            <i v-if="themeStore.themeMode === 'light'" class="i-lucide-sun" />
            <i v-else-if="themeStore.themeMode === 'dark'" class="i-lucide-moon" />
            <i v-else class="i-lucide-contrast" />
          </div>
        </div>
      </button>
    </div>
...
/* 不同主题的图标颜色 */
.theme-toggle-button i.i-lucide-sun {
  color: #f59e0b;
  background-color: currentColor;
}

.theme-toggle-button i.i-lucide-moon {
  color: #6366f1;
  background-color: currentColor;
}

/* mask 图标靠 background-color/currentColor 着色，不能用 background-clip:text */
.theme-toggle-button i.i-lucide-contrast {
  color: #3b82f6;
  background-color: currentColor;
}
...
.handle-icon .i-lucide-sun {
  color: #f59e0b;
  background-color: currentColor;
  filter: drop-shadow(0 0 3px rgba(245, 158, 11, 0.5));
}

.handle-icon .i-lucide-moon {
  color: #fbbf24;
  background-color: currentColor;
  filter: drop-shadow(0 0 3px rgba(251, 191, 36, 0.5));
}

.handle-icon .i-lucide-contrast {
...
/* 色系下拉菜单：走主题 CSS 变量，暗色模式自动适配（scoped 内 @apply dark: 不可靠） */
.color-menu {
  position: absolute;
  left: 0;
  top: 100%;
