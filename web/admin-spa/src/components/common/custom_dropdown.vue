  ...
  <div class="cute-dropdown" :class="[sizeClass, { 'is-open': isOpen, 'is-disabled': disabled }]">
    ...
    >
      <span v-if="icon" class="cute-dropdown__icon" :class="resolvedIconColor">
        <i class="cute-dropdown__glyph" :class="icon" />
      </span>
      ...
      >
        <i class="cute-dropdown__glyph i-lucide-x" />
      </span>

      <span class="cute-dropdown__chevron" :class="{ 'is-open': isOpen }">
        <i class="cute-dropdown__glyph i-lucide-chevron-down" />
      </span>
    </button>
      ...
      >
        ...
        >
          ...
          <div v-if="searchable" class="cute-dropdown__search">
            <i class="cute-dropdown__glyph i-lucide-search cute-dropdown__search-icon" />
            <input
              ref="searchRef"
              v-model="searchQuery"
          ...
          <div ref="listRef" class="cute-dropdown__list">
            <template v-if="filteredOptions.length">
              <div
                ...
                :aria-selected="option.disabled ? undefined : isSelected(option.value)"
              ...
              >
                <i v-if="option.icon" class="cute-dropdown__option-icon cute-dropdown__glyph" :class="option.icon" />
                <span class="cute-dropdown__option-label">{{ option.label }}</span>

                <span
                  v-if="!option.disabled && isSelected(option.value)"
                  aria-hidden="true"
                  class="cute-dropdown__check"
                >
                  <i class="cute-dropdown__glyph i-lucide-check" />
                </span>
              </div>
            </template>
...
const sizeClass = computed(() => `cd-${props.size}`) // 不用 size-*，避免 Uno/Tailwind size 工具类设宽高
...
const selectedLabel = computed(() => {
  if (props.multiple) {
    ...
    if (selectedCount.value === 1) {
      const only = props.options.find((opt) => opt.value === props.modelValue[0])
      return only ? only.label : `已选 1 个`
    }
    return `已选 ${selectedCount.value} 个`
  }
  // isGroup 只是样式分组头，value（如 group-claude）仍可作筛选项
...
const openDropdown = async () => {
  ...
  searchQuery.value = ''
  // 默认高亮当前选中项
  const currentIdx = filteredOptions.value.findIndex((opt) => isSelected(opt.value))
...
const updateDropdownPosition = () => {
  ...
  const availH = placeBelow
    ? window.innerHeight - top - margin
    : trigger.top - margin - gap
  dropdownStyle.value = {
    top: `${top}px`,
    left: `${left}px`,
    // 至少跟触发器同宽；内容更长时随内容撑开，避免短选项菜单被锁成固定宽
    minWidth: minW,
    width: 'max-content',
    maxWidth: `${Math.min(window.innerWidth - margin * 2, 360)}px`,
    maxHeight: `${Math.max(120, Math.min(384, Math.floor(availH)))}px`
  }
}
...
.cute-dropdown__glow {
  ...
  right: -2px;
  top: -2px;
  /* 只贴触发器高度，避免父级被撑高时出现整列蓝条 */
  height: calc(var(--cute-trigger-h, 2.5rem) + 4px);
...
.cd-md .cute-dropdown__trigger {
  height: var(--cute-trigger-h);
  min-height: var(--cute-trigger-h);
  max-height: var(--cute-trigger-h);
  padding: 0 0.75rem;
  box-sizing: border-box;
}

.cd-sm .cute-dropdown__trigger {
  height: var(--cute-trigger-h);
  min-height: var(--cute-trigger-h);
  max-height: var(--cute-trigger-h);
  padding: 0 0.625rem;
...
/* 固定 mask 图标盒模型，防止把 trigger 撑高 */
.cute-dropdown__glyph {
...
.cute-dropdown__panel {
  ...
  overflow: hidden;
  /* 高度只由内容撑开；上限交给 inline maxHeight，避免双 max-height 留底空 */
  height: auto;
...
.cute-dropdown__option:not(.is-disabled):hover,
.cute-dropdown__option.is-highlighted:not(.is-disabled) {
...
/* ---- 暗黑模式（html.dark；Teleport 到 body 也生效） ---- */
html.dark .cute-dropdown__trigger,
...
html.dark .cute-dropdown__trigger:hover,
.dark .cute-dropdown__trigger:hover {
  border-color: rgb(107 114 128 / 0.85);
  background: rgb(31 41 55);
}

html.dark .cute-dropdown__trigger.is-placeholder .cute-dropdown__label,
.dark .cute-dropdown__trigger.is-placeholder .cute-dropdown__label {
  color: rgb(156 163 175);
}

html.dark .cute-dropdown__clear,
.dark .cute-dropdown__clear {
  background: rgb(55 65 81);
  color: rgb(156 163 175);
}

html.dark .cute-dropdown__clear:hover,
.dark .cute-dropdown__clear:hover {
  background: rgb(75 85 99);
  color: rgb(209 213 219);
}

html.dark .cute-dropdown__chevron,
.dark .cute-dropdown__chevron {
  color: rgb(156 163 175);
}

html.dark .cute-dropdown__panel,
...
html.dark .cute-dropdown__search-input,
...
html.dark .cute-dropdown__search-input:focus,
.dark .cute-dropdown__search-input:focus {
  background: rgb(17 24 39);
  border-color: var(--cute-accent, #60a5fa);
}

html.dark .cute-dropdown__search-icon,
.dark .cute-dropdown__search-icon {
  color: rgb(156 163 175);
}

html.dark .cute-dropdown__option,
.dark .cute-dropdown__option {
  color: rgb(209 213 219);
}

html.dark .cute-dropdown__option:not(.is-disabled):hover,
html.dark .cute-dropdown__option.is-highlighted:not(.is-disabled),
.dark .cute-dropdown__option:not(.is-disabled):hover,
.dark .cute-dropdown__option.is-highlighted:not(.is-disabled) {
  background: color-mix(in srgb, var(--cute-accent, #60a5fa) 16%, transparent);
  color: rgb(243 244 246);
}

html.dark .cute-dropdown__option.is-selected,
.dark .cute-dropdown__option.is-selected {
  background: color-mix(in srgb, var(--cute-accent, #60a5fa) 20%, transparent);
  color: var(--cute-accent, #60a5fa);
}

html.dark .cute-dropdown__option.is-selected.is-highlighted,
html.dark .cute-dropdown__option.is-selected:hover,
.dark .cute-dropdown__option.is-selected.is-highlighted,
.dark .cute-dropdown__option.is-selected:hover {
  background: color-mix(in srgb, var(--cute-accent, #60a5fa) 28%, transparent);
}

html.dark .cute-dropdown__option.is-group,
.dark .cute-dropdown__option.is-group {
  background: rgb(55 65 81 / 0.55);
  color: rgb(243 244 246);
}

html.dark .cute-dropdown__check,
.dark .cute-dropdown__check {
  background: color-mix(in srgb, var(--cute-accent, #60a5fa) 22%, transparent);
  color: var(--cute-accent, #60a5fa);
}

html.dark .cute-dropdown__empty,
.dark .cute-dropdown__empty {
  color: rgb(156 163 175);
}

html.dark .cute-dropdown__list::-webkit-scrollbar-thumb,
.dark .cute-dropdown__list::-webkit-scrollbar-thumb {
  background: rgb(75 85 99);
}

html.dark .cute-dropdown__list::-webkit-scrollbar-thumb:hover,
