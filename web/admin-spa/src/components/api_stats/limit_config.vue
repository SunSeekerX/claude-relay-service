<template>
  <div class="flex h-full flex-col gap-2 sm:gap-3">
    ...
    <div class="card flex h-full flex-col p-3">
      <h3
        class="mb-2 flex items-center text-sm font-bold text-gray-900 dark:text-gray-100 sm:text-base"
      >
        <i class="i-lucide-shield mr-2 text-sm text-red-500 md:mr-3 md:text-base" />
      ...
      <div v-if="multiKeyMode && aggregatedStats" class="space-y-4">
        <!-- API Keys 概况 -->
        <div
          class="rounded-lg bg-gradient-to-r from-blue-50 to-indigo-50 p-3 dark:from-blue-900/20 dark:to-indigo-900/20"
        >
          <div class="mb-3 flex items-center justify-between">
            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">
              <i class="i-lucide-layers mr-2 text-blue-500" />
              API Keys 概况
            </span>
            <span
        ...
        <!-- 聚合统计数据 -->
        <div
          class="rounded-lg bg-gradient-to-r from-purple-50 to-pink-50 p-3 dark:from-purple-900/20 dark:to-pink-900/20"
        >
          <div class="mb-3 flex items-center">
            <i class="i-lucide-chart-pie mr-2 text-purple-500" />
            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">聚合统计摘要</span>
          </div>
          <div class="space-y-2">
            <div class="flex items-center justify-between">
              <span class="text-sm text-gray-600 dark:text-gray-400">
                <i class="i-lucide-database mr-1 text-gray-400" />
                总请求数
              </span>
            ...
            <div class="flex items-center justify-between">
              <span class="text-sm text-gray-600 dark:text-gray-400">
                <i class="i-lucide-coins mr-1 text-yellow-500" />
                总 Tokens
              </span>
              <span class="text-sm font-medium text-gray-900 dark:text-gray-100">
            ...
            <div class="flex items-center justify-between">
              <span class="text-sm text-gray-600 dark:text-gray-400">
                <i class="i-lucide-dollar-sign mr-1 text-green-500" />
                总费用
              </span>
              <span class="text-sm font-medium text-gray-900 dark:text-gray-100">
        ...
        >
          <i class="i-lucide-triangle-alert mr-2 text-red-600 dark:text-red-400" />
          ...
          <div class="mb-2 flex items-center justify-between">
            ...
            <span class="text-sm text-gray-500 dark:text-gray-400 md:text-sm">
              ...
              <span v-else class="flex items-center gap-1">
                ${{ statsData.limits.currentDailyCost.toFixed(4) }} / <i class="i-lucide-infinity" />
              </span>
            </span>
          </div>
          <div
        ...
        <div>
          <div class="mb-2 flex items-center justify-between">
            ...
            <span class="text-sm text-gray-500 dark:text-gray-400 md:text-sm">
              ...
              <span v-else class="flex items-center gap-1">
                ${{ statsData.limits.currentTotalCost.toFixed(4) }} / <i class="i-lucide-infinity" />
              </span>
            </span>
          </div>
          <div
        ...
        <div class="space-y-4 border-t border-gray-100 pt-3 dark:border-gray-700">
          <div class="flex items-center justify-between">
            ...
            <span class="text-sm font-medium text-gray-900 md:text-base">
              ...
              <span v-else class="flex items-center gap-1">
                <i class="i-lucide-infinity text-gray-400" />
              </span>
            </span>
          </div>
          <div class="flex items-center justify-between">
            ...
            <span class="text-sm font-medium text-gray-900 md:text-base">
              <span v-if="hasModelRestrictions" class="text-orange-600">
                <i class="i-lucide-triangle-alert mr-1 text-sm md:text-sm" />
                限制 {{ statsData.restrictions.restrictedModels.length }} 个模型
              </span>
              <span v-else class="text-green-600">
                <i class="i-lucide-circle-check mr-1 text-sm md:text-sm" />
                允许所有模型
              </span>
            </span>
          </div>
    ...
    <!-- 详细限制信息 -->
    <div v-if="hasModelRestrictions" class="card !overflow-visible p-3">
      <h3
        class="mb-2 flex items-center text-sm font-bold text-gray-900 dark:text-gray-100 sm:text-base"
      >
        <i class="i-lucide-list mr-2 text-sm text-amber-500 md:mr-3 md:text-base" />
        详细限制信息
      </h3>
      ...
      >
        <h4
          class="mb-2 flex items-center text-sm font-bold text-amber-800 dark:text-amber-300"
        >
          <i class="i-lucide-bot mr-1 text-sm md:mr-2 md:text-sm" />
          受限模型列表
        </h4>
        <div class="max-h-64 space-y-1 overflow-y-auto pr-1 md:max-h-80 md:space-y-2">
          ...
          >
            <i class="i-lucide-ban mr-1 text-sm text-red-500 md:mr-2" />
...
import WindowCountdown from '@/components/api_keys/window_countdown.vue'
