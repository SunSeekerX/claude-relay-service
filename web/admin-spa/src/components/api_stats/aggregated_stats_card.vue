<template>
  <div class="card h-full p-3">
    <h3
      class="mb-2 flex flex-col text-sm font-bold text-gray-900 dark:text-gray-100 sm:flex-row sm:items-center sm:text-base"
    >
      <span class="flex items-center">
        <i class="i-lucide-chart-pie mr-2 text-sm text-orange-500 md:mr-3 md:text-base" />
        使用占比
      </span>
    ...
    <div v-if="aggregatedStats && individualStats.length > 0" class="space-y-2">
      <!-- 各Key使用占比列表 -->
    ...
    >
      <div class="text-center">
        <i class="i-lucide-chart-pie mb-1 text-xl" />
        <p>使用占比仅在多Key查询时显示</p>
      </div>
    </div>
    ...
    <div
      ...
      class="flex h-32 items-center justify-center text-sm text-gray-500 dark:text-gray-400"
    >
      <i class="i-lucide-chart-pie mr-2" />
      暂无数据
    </div>
  </div>
</template>
