import { fileURLToPath } from 'node:url'
import path from 'node:path'
import fs from 'node:fs'
import { env } from './env.js'

const __filename = fileURLToPath(import.meta.url)
const __dirname = path.dirname(__filename)

export const config = {
  // 服务器配置
  server: {
    port: parseInt(env.PORT) || 3000,
    host: env.HOST || '0.0.0.0',
    nodeEnv: env.NODE_ENV || 'development',
    trustProxy: env.TRUST_PROXY === 'true'
  },

  // 安全配置
  security: {
    jwtSecret: env.JWT_SECRET || 'CHANGE-THIS-JWT-SECRET-IN-PRODUCTION',
    adminSessionTimeout: parseInt(env.ADMIN_SESSION_TIMEOUT) || 86400000, // 24小时
    apiKeyPrefix: env.API_KEY_PREFIX || 'cr_',
    encryptionKey: env.ENCRYPTION_KEY || 'CHANGE-THIS-32-CHARACTER-KEY-NOW'
  },

  // Redis配置
  redis: {
    host: env.REDIS_HOST || '127.0.0.1',
    port: parseInt(env.REDIS_PORT) || 6379,
    password: env.REDIS_PASSWORD || '',
    db: parseInt(env.REDIS_DB) || 0,
    connectTimeout: 10000,
    commandTimeout: 5000,
    retryDelayOnFailover: 100,
    maxRetriesPerRequest: 3,
    lazyConnect: true,
    enableTLS: env.REDIS_ENABLE_TLS === 'true'
  },

  // 会话管理配置
  session: {
    // 粘性会话TTL配置（小时），默认1小时
    stickyTtlHours: parseFloat(env.STICKY_SESSION_TTL_HOURS) || 1,
    // 续期阈值（分钟），默认0分钟（不续期）
    renewalThresholdMinutes: parseInt(env.STICKY_SESSION_RENEWAL_THRESHOLD_MINUTES) || 0
  },

  // Claude API配置
  claude: {
    apiUrl: env.CLAUDE_API_URL || 'https://api.anthropic.com/v1/messages',
    apiVersion: env.CLAUDE_API_VERSION || '2023-06-01',
    // 专属账号不可用时是否回退到共享池。
    // 默认 false：API Key 绑定了专属账号就必须使用该账号，不可用时直接报错，
    // 否则「限定账号」的语义会被破坏（请求会静默用到别的账号）。
    dedicatedAccountFallback: env.CLAUDE_DEDICATED_ACCOUNT_FALLBACK === 'true',
    betaHeader:
      env.CLAUDE_BETA_HEADER ||
      'claude-code-20250219,oauth-2025-04-20,interleaved-thinking-2025-05-14,fine-grained-tool-streaming-2025-05-14',
    overloadHandling: {
      enabled: (() => {
        const minutes = parseInt(env.CLAUDE_OVERLOAD_HANDLING_MINUTES) || 0
        // 验证配置值：限制在0-1440分钟(24小时)内
        return Math.max(0, Math.min(minutes, 1440))
      })()
    }
  },

  // Bedrock API配置
  bedrock: {
    enabled: env.CLAUDE_CODE_USE_BEDROCK === '1',
    defaultRegion: env.AWS_REGION || 'us-east-1',
    smallFastModelRegion: env.ANTHROPIC_SMALL_FAST_MODEL_AWS_REGION,
    defaultModel: env.ANTHROPIC_MODEL || 'us.anthropic.claude-sonnet-4-20250514-v1:0',
    smallFastModel:
      env.ANTHROPIC_SMALL_FAST_MODEL || 'us.anthropic.claude-3-5-haiku-20241022-v1:0',
    maxOutputTokens: parseInt(env.CLAUDE_CODE_MAX_OUTPUT_TOKENS) || 4096,
    maxThinkingTokens: parseInt(env.MAX_THINKING_TOKENS) || 1024,
    enablePromptCaching: env.DISABLE_PROMPT_CACHING !== '1'
  },

  // 代理配置
  proxy: {
    timeout: parseInt(env.DEFAULT_PROXY_TIMEOUT) || 600000, // 10分钟
    maxRetries: parseInt(env.MAX_PROXY_RETRIES) || 3,
    // 连接池与 Keep-Alive 配置（默认关闭，需要显式开启）
    keepAlive: (() => {
      if (env.PROXY_KEEP_ALIVE === undefined || env.PROXY_KEEP_ALIVE === '') {
        return false
      }
      return env.PROXY_KEEP_ALIVE === 'true'
    })(),
    maxSockets: (() => {
      if (env.PROXY_MAX_SOCKETS === undefined || env.PROXY_MAX_SOCKETS === '') {
        return undefined
      }
      const parsed = parseInt(env.PROXY_MAX_SOCKETS)
      return Number.isFinite(parsed) && parsed > 0 ? parsed : undefined
    })(),
    maxFreeSockets: (() => {
      if (
        env.PROXY_MAX_FREE_SOCKETS === undefined ||
        env.PROXY_MAX_FREE_SOCKETS === ''
      ) {
        return undefined
      }
      const parsed = parseInt(env.PROXY_MAX_FREE_SOCKETS)
      return Number.isFinite(parsed) && parsed >= 0 ? parsed : undefined
    })(),
    // IP协议族配置：true=IPv4, false=IPv6, 默认IPv4（兼容性更好）
    useIPv4: env.PROXY_USE_IPV4 !== 'false'// 默认 true，只有明确设置为 'false' 才使用 IPv6
  },

  // 请求超时配置
  requestTimeout: parseInt(env.REQUEST_TIMEOUT) || 600000, // 默认 10 分钟

  // 使用限制
  limits: {
    defaultTokenLimit: parseInt(env.DEFAULT_TOKEN_LIMIT) || 1000000
  },

  // 日志配置
  logging: {
    level: env.LOG_LEVEL || 'info',
    dirname: path.join(__dirname, '..', 'logs'),
    maxSize: env.LOG_MAX_SIZE || '10m',
    maxFiles: parseInt(env.LOG_MAX_FILES) || 5
  },

  // 系统配置
  system: {
    cleanupInterval: parseInt(env.CLEANUP_INTERVAL) || 3600000, // 1小时
    tokenUsageRetention: parseInt(env.TOKEN_USAGE_RETENTION) || 2592000000, // 30天
    healthCheckInterval: parseInt(env.HEALTH_CHECK_INTERVAL) || 60000, // 1分钟
    timezone: env.SYSTEM_TIMEZONE || 'Asia/Shanghai', // 默认UTC+8（中国时区）
    timezoneOffset: parseInt(env.TIMEZONE_OFFSET) || 8, // UTC偏移小时数，默认+8
    metricsWindow: parseInt(env.METRICS_WINDOW) || 5 // 实时指标统计窗口（分钟）
  },

  // Web界面配置
  web: {
    title: env.WEB_TITLE || 'Claude Relay Service',
    description:
      env.WEB_DESCRIPTION ||
      'Multi-account Claude API relay service with beautiful management interface',
    logoUrl: env.WEB_LOGO_URL || '/assets/logo.png',
    enableCors: env.ENABLE_CORS === 'true',
    sessionSecret: env.WEB_SESSION_SECRET || 'CHANGE-THIS-SESSION-SECRET'
  },

  // LDAP 认证配置
  ldap: {
    enabled: env.LDAP_ENABLED === 'true',
    server: {
      url: env.LDAP_URL || 'ldap://localhost:389',
      bindDN: env.LDAP_BIND_DN || 'cn=admin,dc=example,dc=com',
      bindCredentials: env.LDAP_BIND_PASSWORD || 'admin',
      searchBase: env.LDAP_SEARCH_BASE || 'dc=example,dc=com',
      searchFilter: env.LDAP_SEARCH_FILTER || '(uid={{username}})',
      searchAttributes: env.LDAP_SEARCH_ATTRIBUTES
        ? env.LDAP_SEARCH_ATTRIBUTES.split(',')
        : ['dn', 'uid', 'cn', 'mail', 'givenName', 'sn'],
      timeout: parseInt(env.LDAP_TIMEOUT) || 5000,
      connectTimeout: parseInt(env.LDAP_CONNECT_TIMEOUT) || 10000,
      // TLS/SSL 配置
      tls: {
        // 是否忽略证书错误 (用于自签名证书)
        rejectUnauthorized: env.LDAP_TLS_REJECT_UNAUTHORIZED !== 'false', // 默认验证证书，设置为false则忽略
        // CA证书文件路径 (可选，用于自定义CA证书)
        ca: env.LDAP_TLS_CA_FILE
          ? fs.readFileSync(env.LDAP_TLS_CA_FILE)
          : undefined,
        // 客户端证书文件路径 (可选，用于双向认证)
        cert: env.LDAP_TLS_CERT_FILE
          ? fs.readFileSync(env.LDAP_TLS_CERT_FILE)
          : undefined,
        // 客户端私钥文件路径 (可选，用于双向认证)
        key: env.LDAP_TLS_KEY_FILE
          ? fs.readFileSync(env.LDAP_TLS_KEY_FILE)
          : undefined,
        // 服务器名称 (用于SNI，可选)
        servername: env.LDAP_TLS_SERVERNAME || undefined
      }
    },
    userMapping: {
      username: env.LDAP_USER_ATTR_USERNAME || 'uid',
      displayName: env.LDAP_USER_ATTR_DISPLAY_NAME || 'cn',
      email: env.LDAP_USER_ATTR_EMAIL || 'mail',
      firstName: env.LDAP_USER_ATTR_FIRST_NAME || 'givenName',
      lastName: env.LDAP_USER_ATTR_LAST_NAME || 'sn'
    }
  },

  // 用户管理配置
  userManagement: {
    enabled: env.USER_MANAGEMENT_ENABLED === 'true',
    defaultUserRole: env.DEFAULT_USER_ROLE || 'user',
    userSessionTimeout: parseInt(env.USER_SESSION_TIMEOUT) || 86400000, // 24小时
    maxApiKeysPerUser: parseInt(env.MAX_API_KEYS_PER_USER) || 1,
    allowUserDeleteApiKeys: env.ALLOW_USER_DELETE_API_KEYS === 'true' // 默认不允许用户删除自己的API Keys
  },

  // Webhook通知配置
  webhook: {
    enabled: env.WEBHOOK_ENABLED !== 'false', // 默认启用
    urls: env.WEBHOOK_URLS
      ? env.WEBHOOK_URLS.split(',').map((url) => url.trim())
      : [],
    timeout: parseInt(env.WEBHOOK_TIMEOUT) || 10000, // 10秒超时
    retries: parseInt(env.WEBHOOK_RETRIES) || 3 // 重试3次
  },

  // 开发配置
  development: {
    debug: env.DEBUG === 'true',
    hotReload: env.HOT_RELOAD === 'true'
  },

  // 账户余额相关配置
  accountBalance: {
    // 是否允许执行自定义余额脚本（安全开关）
    // 余额脚本可发任意 HTTP 并执行 extractor，仅建议受控环境开启
    // 默认保持开启；如需禁用请显式设置：BALANCE_SCRIPT_ENABLED=false
    enableBalanceScript: env.BALANCE_SCRIPT_ENABLED !== 'false'
  },

  // 用户消息队列配置
  // 锁在请求发送成功后立即释放；Claude API 限流按发送时刻计
  userMessageQueue: {
    enabled: env.USER_MESSAGE_QUEUE_ENABLED === 'true', // 默认关闭
    delayMs: parseInt(env.USER_MESSAGE_QUEUE_DELAY_MS) || 200, // 请求间隔（毫秒）
    timeoutMs: parseInt(env.USER_MESSAGE_QUEUE_TIMEOUT_MS) || 5000, // 队列等待超时（毫秒），锁持有时间短，无需长等待
    lockTtlMs: parseInt(env.USER_MESSAGE_QUEUE_LOCK_TTL_MS) || 5000 // 锁TTL（毫秒），5秒足以覆盖请求发送
  },

  // 额度卡兑换上限配置（防盗刷）
  quotaCardLimits: {
    enabled: env.QUOTA_CARD_LIMITS_ENABLED !== 'false', // 默认启用
    maxExpiryDays: parseInt(env.QUOTA_CARD_MAX_EXPIRY_DAYS) || 90, // 最大有效期距今天数
    maxTotalCostLimit: parseFloat(env.QUOTA_CARD_MAX_TOTAL_COST_LIMIT) || 1000 // 最大总额度（美元）
  },

  // 上游错误自动暂停配置
  // 全局默认；Claude OAuth 账号可后台覆盖 503/5xx，或禁用 temp_unavailable（账号策略优先）
  upstreamError: {
    serviceUnavailableTtlSeconds: parseInt(env.UPSTREAM_ERROR_503_TTL_SECONDS) || 60, // 503错误暂停秒数
    serverErrorTtlSeconds: parseInt(env.UPSTREAM_ERROR_5XX_TTL_SECONDS) || 300, // 5xx错误暂停秒数
    overloadTtlSeconds: parseInt(env.UPSTREAM_ERROR_OVERLOAD_TTL_SECONDS) || 600, // 529过载暂停秒数
    authErrorTtlSeconds: parseInt(env.UPSTREAM_ERROR_AUTH_TTL_SECONDS) || 1800, // 401/403认证错误暂停秒数
    timeoutTtlSeconds: parseInt(env.UPSTREAM_ERROR_TIMEOUT_TTL_SECONDS) || 300, // 504超时暂停秒数
    // 上游 retry-after 派生的暂停时长上限（秒）。周级限额的 retry-after 可达数天，
    // 若不钳制会把账号整整下线数天。长时限额应由账号级/模型级限流桶承担。
    maxCustomTtlSeconds: parseInt(env.UPSTREAM_ERROR_MAX_CUSTOM_TTL_SECONDS) || 1800
  }
}

